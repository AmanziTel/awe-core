...
int _C____ =3, _S___ =2, _N_____=1, _N____=2, _O____=1, ReCalAll=0, UTraf=0,
		USON=0, QUALITY=50, DECOMP=1, ExCliq=0, HType, UGroup, NOGR, CCard;
NOFF, NOC, CMaxRt, * FC____;
char C_F___[MaxFileName+1], N_F___[MaxFileName+1], I_F___[MaxFileName+1],
		P_F___[MaxFileName+1], ///?planfile
		ClF___[MaxFileName+1], PrF___[MaxFileName+1], ///?protfile
		F_F___[MaxFileName+1], E_F___[MaxFileName+1];
#define Q_(a)                    pow(a,2)
...
void a_____e___(int is_fix, T_N__ * xnet) {
	...
	if (is_fix)
		...
		xnet->B = 2*(FBANDL+MAX_(MAXDIS,_S___)) + 5;
...
int g___n___(char *snxx, int ssx) {
	int i;
	for (i=0; i<_N______->Card; i++)
		if ( (_N______->_N___[i]->TSS == ssx) && !strcmp(snxx, 
		_N______->_N___[i]->S) )
...
void w_____c_______(char * fname, T_C___ * xN) {
	...
	while (j<CARD_(_N______)) {
		fprintf(cefp, "%s %d %d %d %d ", _N______->_N___[j]->S, 
		_N______->_N___[j]->TSS, _N______->_N___[j]->SS, 
		_N______->_N___[j]->NC, _N______->_N___[j]->AxF);
...
void Extend_clqset(BOOL * foundp, int priop, int ubp, int * rankp, int * xlbp,
	...
	p=card_xQ;
	while ((p < CARD_(_S____) ) && (NoCh || (!*foundp && (ylb < (ubp+1)) && LBfilter(
...
int get_MLB(int Minprio, int Ncar, int Mindist, int Ldis, ___P____ ** rel,
	...
	fprintf(
			Protfp,
			"The %d maximal clique has Lower Bound = %2d and %d following sectors:\n{",
	...
	while (j<NOFC) {
		for (i=0; i<CARD_(aux); i++) {
			...
			if (E_(i,aux)) {
				...
				if (l<k) {
					cet[j].Fn = _N______->_N___[_S_____(xQ0->Chain[i])]->ff[l];
					cet[j].fxd = 1;
				} else {
					cet[j].Fn = -1;
					cet[j].fxd = 0;
...
int RhoCC(int ix, T_S__ * xX) {
	...
	VV(ix, xX);
	GFB(_N______->lb,_N______->ub,ix);
	if (FROL.frno >= 0)
		return (0);
	GFL(_N______->lb,ix);
	a=FROL.frno;
	...
	if (a != _N______->B) {
		if (b != -1) {
			...
			if ((_N______->lb - a - FROL.frdis) < (b - _N______->ub
					- FROR.frdis))
				b = _N______->ub + 1 - a;
			else
				b = b + 1 - _N______->lb;
...
int RRRX(int nix, T_S__ * xX) {
	...
	FREQ(nix) = -1;
	VV(nix, xX);
	GFB(_N______->lb,_N______->ub,nix);
	if (FROL.frno < 0)
		return (0);
	FREQ(nix) = FROL.frno;
	XC(NIX(nix))--;
...
void RR(int nix, T_S__ * xX) {
	...
	if (FROL.frno < _N______->B) {
		if (FROR.frno > -1) {
			if ((_N______->lb - FROL.frno - FROL.frdis) < (FROR.frno
					- _N______->ub - FROR.frdis)) {
				FREQ(nix) = FROL.frno;
				_N______->lb = MIN_(_N______->lb,FROL.frno);
				XC(NIX(nix))--;
				FBAND = True && ((_N______->ub - FROL.frno + 1) < FBANDL);
				return;
			}
			FREQ(nix) = FROR.frno;
			_N______->ub = MAX_(_N______->ub,FROR.frno);
			XC(NIX(nix))--;
...
void set_cr_SCC_it(int o_pri, int pri, int dis, int ctre, int atre,
	...
	ALLN = 0;
	for (nix=0; nix<_N______->Card; nix++)
		for (njx=nix+1; njx<_N______->Card; njx++) {
			_P__(CRX,nix,njx,d1)
			;
			if (!(d1 < o_pri))
				continue;
...
void set_step(int prio) {
	...
	while (PPar7[prio].pclb < AT) {
		...
		switch (opt_all_it(QC, prio, ____X, prio-1, PPar7[prio].pdisr,
				PPar7[prio].pdisr-1, AT, _N______->_E___)) {
		case 0:
			AT--;
			QC = MAX_(QC,MIN_SET);
			break;
