...
public class JSONReader {
    ...
    private URL dataURL;    // optional data URL in case it was automatically determined from header URL (or vice versa)
	...
	/**
	 * Create a JSON reader on the specified URL.
	 * This class opens the URL connection on demand for any data request. The URL is not
	 * checked until opened, so if it is likely to be invalid, test before this point.
	 * 
	 * @param url
	 */
	...
	/**
	 * This method is used internally by the other data methods, and caches its
	 * results for future queries. It is available here for code that wishes to
	 * investigate the JSON more specifically.
	 * 
	...
	/**
	 * Find the Coordinate Reference System in the JSON, or default
    ...
    /**
     * Find the Coordinate Reference System in the JSON, or default
     * to the specified default if no CRS is found in the JSON.
     * @return CoordinateReferenceSystem
     */
	    ...
	    if(crs==null){
            ...
            try{
                ...
                if(jsonCRS.getString("type").equals("name")){
                    // The simple approach is to name the CRS, eg. EPSG:4326 (GeoJSON spec prefers a new naming standard, but I'm not sure geotools knows it)
                    crs = CRS.decode(jsonCRSProperties.getString("name"));
                }else if(jsonCRS.getString("type").equals("link")){
                    // TODO: This type is specified in GeoJSON spec, but what the HREF means is not, so we assume it is a live URL that will feed a CRS specification directly
    ...
    /**
     * Find the bounding box for the data set as a ReferenceEnvelope. It uses the
     * getCRS method to find the reference system then looks for explicit "bbox"
     * elements, and finally, if no bbox was found, scans all feature geometries
     * for coordinates and builds the bounds on those. The result is cached for
     * future calls.
	    ...
	    if(bounds==null){
            ...
            try{
                if(this.bounds.isNull()){
                    // Try to create envelope from any data contained in the GeoJSON object (fails for header, but then that should have a bbox)
                        ...
                        for(int i=0;i<features.size();i++){
                            ...
                            if(geometry!=null){
                                ...
                                if(geometryType.equals("Point")){
                                    this.bounds.expandToInclude(coordinates.getDouble(0), coordinates.getDouble(1));
                                }else{
                                    for(int x=0;x<coordinates.size();x++){
                                        JSONArray coords = coordinates.getJSONArray(x);
                                        this.bounds.expandToInclude(coords.getDouble(0), coords.getDouble(1));
                                    }
                                }
                            }else{
                                System.err.println("Failed to find geometry in feature: "+feature.toString());
                            }
                        }
                    }
                }
    ...
    /**
     * Return the name of the dataset as specified in the JSON, or default
     * to the URL.getFile().
     * @return dataset name
     */
	...
	/**
	 * This is the based class for all features that are genuinely build from JSON. As
	 * such a JSONObject is passed to the constructor, and used to generate the feature
	 * data required.
	 * @author craig
	 */
    public static class JSONFeature implements Feature {
        ...
        public JSONFeature(JSONObject jsonObject){
            ...
            this.type = this.geometry.getString("type");    // We only care about the geometry type, because the feature type is by definition "Feature"
        }
        public String getType() {
            return type;
        }
        ...
        public String toString(){
            if(getProperties().containsKey("name")) return getProperties().get("name").toString();
            else return points[0].toString();
        }
    }
    public static class SimplePointFeature implements Feature{
        ...
        public String toString(){
            if(properties.containsKey("name")) return properties.get("name").toString();
            else return point.toString();
        }
    }

	/**
	 * Interface for various anonymous inner classes that can
	 * produce a stream of JSON features based on the internal JSON data.
	 * This allows, for example, the JSON to contain the features directly
	 * as well as reference another data source, like a file or
	 * another URL, that will provide the necessary data.
	 * @author craig
	 */
	public static interface FeatureReader {
        public Feature getFeature();
	}
	private static class JSONFeatureReader implements FeatureReader {
    ...
    private static class JSONURLFeatureReader extends JSONFeatureReader {
        ...
        public Feature getFeature() {
            ...
            return super.getFeature();  // loops through JSONArray
        }
    }
    private static class CSVURLFeatureReader implements FeatureReader {
        private URL feature_url;
        ...
        public Feature getFeature() {
            if(reader==null){
                try {
                    ...
                    for(String header:reader.getHeaders()) headers.put(header.toLowerCase(), headers.size());
                    for(String head:new String[]{"long","longitude","x"}){
                        if(headers.containsKey(head)) x_col = headers.get(head);
                    }
                    for(String head:new String[]{"lat","latitude","y"}){
                        if(headers.containsKey(head)) y_col = headers.get(head);
                    }
                    for(String head:new String[]{"description","name"}){
                        if(headers.containsKey(head)) name_col = headers.get(head);
                    }
                    // test for invalid x and y columns
                    if(x_col < 0 || x_col >= reader.getHeaderCount()) throw new Exception("Invalid easting column: "+x_col);
                    if(y_col < 0 || y_col >= reader.getHeaderCount()) throw new Exception("Invalid northing column: "+y_col);
                    ...
                    if(invalidNameCol()) name_col = -1; // deal with this later
                } catch (Exception e) {
                    System.err.println("Failed to get features from url '"+feature_url+"': "+e);
                    e.printStackTrace(System.err);
                }
            ...
            try{
                ...
                for(int i=0;i<reader.getColumnCount();i++) {
                    if(i!= x_col && i!=y_col && i!=name_col) properties.put(reader.getHeader(i), reader.get(i));
                }
	    ...
	    if(features!=null){
            ...
            try {
                ...
                if(dataType!=null && dataType.toLowerCase().equals("csv")){
                    return new CSVURLFeatureReader(feature_url);    // Connect to a URL and interpret as CSV stream
                } else if(dataType!=null && dataType.toLowerCase().endsWith("json")){   // support .json and .geo_json
                    return new JSONURLFeatureReader(feature_url);   // Connect to a URL and interpret as JSON stream
                }else if(this.dataURL!=null){
                    return new JSONURLFeatureReader(dataURL);   // Connect to data URL if specified, and assume it's JSON (we get here if the data URL is a simple modification of the header URL)
                }else{
                    System.err.println("JSON URL contained no features, nor a reference to an alternative feature source: "+this.url);
                    return null;
                }
	        ...
	        // TODO: Support geometries collection also (like features but without bbox and properties)
            System.err.println("JSON contains no features collection, or a featues_source reference");
            return null;
	    }
	}
}
