...
public class AbstractRenderer extends RendererImpl {
	...
	public void setScaling(Envelope bounds_transformed, Envelope data_bounds,
		...
		if (RenderOptions.scale.equals(Scale.LARGE)
				&& RenderOptions.scaleSymbols) {
			RenderOptions.large_sector_size *= Math
					.sqrt(RenderOptions.maxSitesFull)
					/ (3 * Math.sqrt(countScaled));
			RenderOptions.large_sector_size = Math.min(
					RenderOptions.large_sector_size,
					RenderOptions.maxSymbolSize);
		}
	...
	private void renderGeoResource(Graphics2D destination,
		...
		try {
			...
			IRenderableModel model = resource.resolve(IRenderableModel.class,
					monitor);
			...
			if (bounds_transformed == null) {
				...
				long count = ((INetworkModel) model)
