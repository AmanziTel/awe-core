...
public class NetworkRenderer extends AbstractRenderer {
		...
		for (IDataElement currentElement : model.getSelectedElements()) {
			...
			if (point != null) {
				if (networkRendererStyle.getScale() == Scale.LARGE) {
					...
					sectorRendering(destination, point, model, site,
							currentElement, true);
					renderSectorNeighbours(destination, point,
							(INetworkModel) model, site, currentElement,
							bounds_transformed);
				}
			} else {
				continue;
	...
	private Arc2D createSector(Point point, double radius, double angle,
		...
		sector.setArcByCenter(getSectorXCoordinate(point),
				getSectorYCoordinate(point), radius, angle, beamwidth,
				Arc2D.OPEN);
		return sector;
	}
	...
	private void renderSelectionGlow(Graphics2D g, java.awt.Point p,
		...
		for (; drawSize > 2; drawSize *= 0.8) {
			g.fillOval(p.x - drawSize - 9, p.y - drawSize - 9, 2 * drawSize,
					2 * drawSize);
		}
	}
}