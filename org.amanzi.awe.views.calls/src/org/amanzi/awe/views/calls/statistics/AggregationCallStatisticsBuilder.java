...
public class AggregationCallStatisticsBuilder {
    ...
    private InclInconclusiveStates inconclusive;
     ...
     */
    public AggregationCallStatisticsBuilder(Node datasetNode,GraphDatabaseService neo, InclInconclusiveStates isInconclusive) {
        dataset = datasetNode;
        service = neo;
            ...
            for(Node row : currSourceRows){
                ...
                for (IStatisticsHeader utilHeader : stat.getUtilHeaders()) {
                    for (IStatisticsHeader header : ((IAggrStatisticsHeaders)utilHeader).getDependendHeaders()) {
                        ...
                        if (cell!=null) {
                            Number value = utilHeader.getStatisticsData(cell, null, null);
    ...
    private Node createRootStatisticsNode() {
        ...
        result.setProperty(INeoConstants.PROPERTY_IS_INCONCLUSIVE, !inconclusive.equals(InclInconclusiveStates.EXCLUDE));
        result.setProperty(INeoConstants.PROPERTY_INCONCLUSIVE_STATE, inconclusive.getId());
