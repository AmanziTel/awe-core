 ...
 * @since 1.0.0
 */
public class RenderMetricsFactory implements IRenderMetricsFactory {

    @Override
    public boolean canRender(IRenderContext context) throws IOException {
        for (IGeoResource resource : context.getLayer().getGeoResources()) {
            if (resource.canResolve(GeoNeo.class)) {
                return resource.resolve(GeoNeo.class, null).getGisType() == GisTypes.Star;
            }
        }
        return false;
    }
    ...
    public IRenderMetrics createMetrics(IRenderContext context) {
        return new StarRendererMetrics(context, this);
    }

    @Override
    public Class< ? extends IRenderer> getRendererType() {
        return StarRenderer.class;
    }
