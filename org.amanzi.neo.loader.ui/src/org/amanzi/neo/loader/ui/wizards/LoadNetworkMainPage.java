...
public class LoadNetworkMainPage extends LoaderPageNew<ConfigurationDataImpl> {
	private static final Logger LOGGER = Logger
			.getLogger(LoadNetworkMainPage.class);
	/*
	...
	public void createControl(Composite parent) {
		...
		editor = new FileFieldEditorExt(
				"fileSelectNeighb", NeoLoaderPluginMessages.NetworkSiteImportWizard_FILE, main); // NON-NLS-1 //$NON-NLS-1$
		...
		loaderType.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 2, 3));
	 ...
	 */
	protected ILoaderNew<? extends IData, ConfigurationDataImpl> setFileName(
			String fileName) {
	...
	private String[] getRootItems() {
		...
		try {
			...
			getNewConfigurationData().getDatasetNames().put(
					ConfigurationDataImpl.PROJECT_PROPERTY_NAME,
	...
	protected void changeNetworkName() {
		...
		getNewConfigurationData().getDatasetNames().put(
				ConfigurationDataImpl.NETWORK_PROPERTY_NAME, networkName);
	...
	protected boolean validateConfigData(ConfigurationDataImpl configurationData) {
		...
		if (fileName == null) {
			setMessage(
					NeoLoaderPluginMessages.NetworkSiteImportWizardPage_NO_FILE,
					DialogPage.ERROR);
			return false;
		}
		...
		if (StringUtils.isEmpty(networkName)) {
			setMessage(
					NeoLoaderPluginMessages.NetworkSiteImportWizardPage_NO_NETWORK,
					DialogPage.ERROR);
			return false;
		}
		if (getNewSelectedLoader() == null) {
			setMessage(
					NeoLoaderPluginMessages.NetworkSiteImportWizardPage_NO_TYPE,
					DialogPage.ERROR);
			return false;
		}
		IValidateResult.Result result = getNewSelectedLoader().getValidator()
				.isValid(configurationData);
		if (result == Result.FAIL) {
			setMessage(String.format(getNewSelectedLoader().getValidator()
					.getMessages(), getNewSelectedLoader().getLoaderInfo()
