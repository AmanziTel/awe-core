...
public class NetworkSectorDataValidator implements ILoaderInputValidator<CommonConfigData>{
    ...
    public IValidateResult validate(CommonConfigData data) {
        try {
            ...
            int sectorHeader=LoaderUtils.findHeaderId(header,LoaderUiUtils.getPossibleHeaders(DataLoadPreferences.NH_SECTOR),0);
            if (sectorHeader>=0){
                int latHeader=LoaderUtils.findHeaderId(header,LoaderUiUtils.getPossibleHeaders(DataLoadPreferences.NH_LATITUDE),0);
                if (latHeader>=0){
