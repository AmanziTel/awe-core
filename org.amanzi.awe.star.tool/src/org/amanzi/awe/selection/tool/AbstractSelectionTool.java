...
public abstract class AbstractSelectionTool extends AbstractModalTool {
	...
	private static final Logger LOGGER = Logger
			.getLogger(AbstractSelectionTool.class);
	...
	public void mouseReleased(MapMouseEvent event) {
		...
		Envelope selectionBounds = new Envelope(startCoordinate, getContext()
	...
	protected IDataModel getAnalyzedModel() {
		for (ILayer layer : context.getMapLayers()) {
			...
			try {
				...
				LOGGER.error("Selection Tool: resolved resource was unavailable due to a technical problem. "
						+ e);
			}
		}
		return null;
	...
	protected PropertiesView getPropertiesView() {
		...
		PropertiesView resultView = null;
		try {
			if ((resultView = (PropertiesView) page
					.findView(PROPERTIES_VIEW_ID)) == null) {
				resultView = (PropertiesView) page
	 ...
	 * @param point
	 *            Contains coordinates of clicked element on map.
	 */
	protected abstract void handleSelection(IDataModel model,
			Envelope selectionBounds, Point point);
}
