...
public class ElementsSelectionTool extends AbstractSelectionTool {
     ...
     * Logger instance
     */
    private static final Logger LOGGER = Logger.getLogger(ElementsSelectionTool.class);
    ...
    protected void handleSelection(IDataModel model, Envelope selectionBounds, Point point) {
        if (model instanceof IDriveModel) {
            ...
            try {
                for (IDataElement selectedElement : driveModel.getElements(selectionBounds)) {
                    driveModel.setSelectedDataElementToList(selectedElement);
                }
                fireEvents(driveModel);
            } catch (AWEException e) {
                LOGGER.error("Elements Selection Tool: cannot get elements by bounds. " + e);
            }
        }
    }
