...
public class IRBUtils {
	private static IRBUtils instance = new IRBUtils();
	...
	private List<String> doMakeLoadPath(String[] extras) {
        ...
        loadPath.add(userDir+"/.awe/script");
        loadPath.add(userDir+"/.awe/lib");
	...
	private String ensureJRubyHome() {
		if(jRubyHome==null){
			jRubyHome = IRBUtils.findJRubyHome(System.getProperty("jruby.home"));
		}
		return(jRubyHome);
	}
	...
	private String ensureJRubyVersion() {
		if(jRubyVersion==null){
			jRubyVersion = IRBUtils.findJRubyVersion(ensureJRubyHome(),System.getProperty("jruby.version"));
		}
		return(jRubyVersion);
	}
