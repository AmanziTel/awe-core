 ...
 * @author Bondoronok_P
 */
public class DriveTreeContentProvider implements IStructuredContentProvider,
	...
	public Object[] getChildren(Object parentElement) {
		...
		if (parentElement instanceof IDriveModel) {
			...
			return isPreviousPressed() ? tree.getPreviousElements().toArray()
					: tree.getNextElements().toArray();
	...
	public boolean hasChildren(Object parentElement) {
		...
		if (parentElement instanceof IDriveModel) {
			...
			String name = (String) ((IDataElement) parentElement)
					.get(AbstractService.NAME);
			if (name != null && Messages.NextElementsTitle.equals(name)
					|| Messages.PreviousElementsTitle.equals(name)) {
				return true;
			}
		}
		return false;
	...
	public Object[] getElements(Object inputElement) {
		...
		try {
			driveModels = ProjectModel.getCurrentProjectModel()
					.findAllDriveModels();
		} catch (AWEException e) {
			MessageDialog.openError(null, Messages.ErrorTitle,
					Messages.CouldNotGetAllDriveModels);
			return null;
		}
		...
		for (IDriveModel model : driveModels) {
			networkModelsInObjects = ArrayUtils.add(networkModelsInObjects,
					model);
			EventManager.getInstance().fireEvent(
					new AnalyseEvent(model, DriveTreeView.DRIVE_TREE_VIEW_ID));
		}
	...
	private static class VirtualTree {
		...
		/**
		 * Get VirtualTree instance NOTE: After you get virtual table instance
		 * you must initialize iterator, by call initializeIterator() method
		 * 
		 * @return VirtualTree
		 */
		 ...
		 */
		public void initalizeIterator(Iterable<IDataElement> loadedElements,
		...
		/**
		 * Get next 20 elements. Before using this method virtual table must be
		 * initialized
		 * 
		...
		/**
		 * Get previous 20 elements. Before using this method virtual table must
		 * be initialized
		 * 
		...
		/**
		 * Put Previous elements IDataElement in case if iterator has previous
		 * items
		 * 
		...
		private List<IDataElement> initializeElementsSet() {
			...
			if (isNotRoot()) {
				...
				parentItemProperties.put(AbstractService.NAME,
						Messages.PreviousElementsTitle);
				parentItemProperties
						.put(AbstractService.TYPE, DriveNodeTypes.M);
