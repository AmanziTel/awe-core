 ...
 */
public class FilterCreationPage {
    private static final Logger LOGGER = Logger.getLogger(FilterCreationPage.class);
    /*
     * constants
     */
    ...
    public FilterCreationPage(Shell parentShell, IRenderableModel model) {
    ...
    protected void addFilter() {
        ...
        try {
            ...
            LOGGER.error("error while try to create filter from collected parameters", e);
            return;
        }
    ...
    private void setFilterValues(IFilter filter) {
        INodeType nType = NodeTypeManager.getType(propCollector.get(NODE_TYPES));
        String propertyName = propCollector.get(PROPERTY_NAME);
        Object value = parseValue(propCollector.get(VALUE));
        filter.setExpression(nType, propertyName, (Serializable)value);
    }

    /**
     * check for correct value input if evrithyng is Ok return true else return false and show error
     * dialog and reset value field;
     * 
     * @return
     */
    private boolean isValid() {
        ...
        Class< ? > propertyClass = castedModel.getPropertyClass(nType, propertyName);

        if (value.getClass() != propertyClass) {
            MessageDialog.openError(
                    shell,
                    "Incorect property format",
                    String.format("%s should store property instance of %s instance but not %s", propertyName, propertyClass,
                            value.getClass()));
            tValue.setText(StringUtils.EMPTY);
            updateDescriptionField();
            return false;
     ...
     * @return
     */
    protected Object parseValue(String propertyValue) {
        try {
