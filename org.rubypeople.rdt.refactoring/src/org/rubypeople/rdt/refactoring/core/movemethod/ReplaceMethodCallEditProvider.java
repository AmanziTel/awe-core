...
public class ReplaceMethodCallEditProvider extends ReplaceEditProvider {
	...
	protected int getOffsetLength() {
		return methodCallNode.getPosition().getEndOffset() - methodCallNode.getPosition().getStartOffset();
	}

	@Override
	protected Node getEditNode(int offset, String document) {
		Node receiverNode;
		if(!methodCallNode.isCallToClassMethod()) {
		...
		CallArgsNodeWrapper argsNode = new CallArgsNodeWrapper(methodCallNode.getArgsNode());
		if(config.doesNewMethodNeedsReferenceToSourceClass()) {
			SelfNode selfNode = NodeFactory.createSelfNode();
			return NodeFactory.createCallNode(receiverNode, config.getMovedMethodName(), argsNode.cloneWithAddedArg(selfNode));
		}
		return NodeFactory.createCallNode(receiverNode, config.getMovedMethodName(), argsNode.getWrappedNode());
	}

	@Override
	protected int getOffset(String document) {
		return methodCallNode.getPosition().getStartOffset();
	}
