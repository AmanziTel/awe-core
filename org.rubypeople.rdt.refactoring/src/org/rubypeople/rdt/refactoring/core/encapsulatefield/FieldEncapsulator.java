...
public class FieldEncapsulator extends MultiEditProvider {
	...
	private void initVisibilities() {
		writerVisibility = (!config.hasSelectedAccessor() || !config.getSelectedAccessor().isWriter() ? VisibilityNodeWrapper.METHOD_VISIBILITY.PRIVATE : VisibilityNodeWrapper.METHOD_VISIBILITY.PUBLIC);
		readerVisibility = (!config.hasSelectedAccessor() || !config.getSelectedAccessor().isReader() ? VisibilityNodeWrapper.METHOD_VISIBILITY.PRIVATE : VisibilityNodeWrapper.METHOD_VISIBILITY.PUBLIC);
	}
	...
	private InsertEditProvider getInsertEditProvider(final boolean writer, final VisibilityNodeWrapper.METHOD_VISIBILITY visibility) {
		return new InsertEditProvider(true) {
			...
			protected int getOffset(String document) {
				IOffsetProvider offsetProvider = new AfterLastMethodInClassOffsetProvider(config.getEnclosingClassNode(), document);
	...
	public String getSelectedFieldName() {
		return (config.hasSelectedAccessor()) ? '@' + config.getSelectedAccessor().getAttrName() : config.getSelectedInstNode().getName();
	}
