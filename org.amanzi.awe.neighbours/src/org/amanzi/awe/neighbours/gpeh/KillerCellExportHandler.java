...
public class KillerCellExportHandler extends IntraModelHandler {
	 ...
	 *            the service
	 */
	public KillerCellExportHandler(CallTimePeriods period,
	...
	public boolean setData(CellNodeInfo bestCell, InterfCellInfo interfCell) {
		...
		String type = bestCell.getCellSectorInfo()
				.getProperty(STATISTIC_PROPERTY_TYPE).toString();
		String name = bestCell.getCellSectorInfo()
				.getProperty(INeoConstants.PROPERTY_NAME_NAME).toString();
		Node checkNode = bestCell.getCellSectorInfo();
		while (checkNode.hasRelationship(Direction.OUTGOING)
				&& checkNode.getProperty(INeoConstants.PROPERTY_NAME_NAME)
						.toString().equals(name)) {
			Node n = checkNode.getSingleRelationship(
					NetworkRelationshipTypes.NEXT, Direction.OUTGOING)
					.getEndNode();
			if (checkNode.getProperty(STATISTIC_PROPERTY_TYPE).equals(type)) {
				statList.add(n);
			}

			checkNode = n;
		}
		Iterator<Node> iterator = statList.iterator();
		// Pattern pat=Pattern.compile("^(\\D)(\\d)$");
		Pattern pat = Pattern.compile("^(numMrForBestCellIntra)(\\d+)$");
		Set<Long> timestamps = defineTimestamps(bestCell.getCellSectorInfo(),
				pat, 2);
		...
		while (iterator.hasNext()) {
			Node interNode = iterator.next();
			int[] values = new int[5];
			computeArrayValue(values, interNode, paramIntraEcnoListDelta,
					timestamps);
