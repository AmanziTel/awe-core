...
public class ExportProvider3GPP extends AbstractGpehExportProvider {
	...
	private static final Logger LOGGER = Logger
			.getLogger(ExportProvider3GPP.class);
	 ...
	 *            the dataname
	 */
	public ExportProvider3GPP(Node dataset, Node network,
			GraphDatabaseService service, ValueType value3gpp,
			RelationshipType statRelation, CallTimePeriods period,
	 ...
	 *            the model
	 */
	private void updateModel(Node statNode, Long computeTime,
			Model3GPPValue model) {
//		statNode = statNode.getSingleRelationship(
//				NetworkRelationshipTypes.CHILD, Direction.OUTGOING)	.getEndNode();
		Traverser td = Traversal
				.description()
				.depthFirst()
				.relationships(GeoNeoRelationshipTypes.NEXT, Direction.OUTGOING)
				.evaluator(new BestCellEvaluator(statRelation.name())).traverse(statNode.getSingleRelationship(
						NetworkRelationshipTypes.CHILD, Direction.OUTGOING)	.getEndNode());
		Iterator<Node> nodeIter = td.nodes().iterator();
		Node checkNode;
		while (nodeIter.hasNext()) {
			checkNode = nodeIter.next();
			Pair<Integer, Integer> ciRnc = getCiRncPair(checkNode);
			Node bestCell = NeoUtils.findSector(network, ciRnc.getLeft(),
					String.valueOf(ciRnc.getRight()), luceneService, service);
			if (bestCell == null) {
				LOGGER.warn(String
						.format("Data not included in statistics! Not found sector with ci=%s, rnc=%s",
								ciRnc.getLeft(), ciRnc.getRight()));
				continue;
			}
			Node bestCellNode = checkNode;
			for (String propertyName : bestCellNode.getPropertyKeys()) {
				if (propertyName.equals(INeoConstants.PROPERTY_TIMESTAMP_NAME)) {
						...
						if (bestCellNode.hasProperty("Range size")) {
							...
							for (int i = 0; i < values.length; i++) {
								if (bestCellNode.hasProperty(statRelation
										.name() + " " + i)) {
									values[i] = (Integer) bestCellNode
											.getProperty(statRelation.name()
													+ " " + i);
								}
							}
	 ...
	 */
	private Pair<Integer, Integer> getCiRncPair(Node node) {

		String[] ciRnc = node.getProperty(INeoConstants.PROPERTY_NAME_NAME)
				.toString().split("_");
