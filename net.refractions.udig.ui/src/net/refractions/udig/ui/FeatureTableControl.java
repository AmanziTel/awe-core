...
public class FeatureTableControl implements ISelectionProvider {
     ...
     */
    public FeatureTableControl( final IProvider<IProgressMonitor> monitorProvider,
            Composite parent, FeatureCollection features ) {
        ...
        if (features instanceof IAdaptable
            ...
            for( int i = 0; i < properties.length; i++ ) {
                if (i == 0)
                    ...
                    properties[i] = schema.getAttributeType(i - 1).getName();
                }
            }
    ...
    private void addSelectionListener( final Table table ) {
        ...
        table.addListener(SWT.MouseDown, new Listener(){
            ...
            private void handleSelecteRange( final Table table, int index, FeatureTableContentProvider provider, Collection<String> selectionFids ) {
                ...
                List<Feature> toAdd = provider.features.subList(low, high+1);
    ...
    private void setCellValidators( IAdaptable adaptable ) {
        ...
        for( int i = 0; i < editors.length - offset; i++ ) {
            ...
            if (validators != null && validators[i] != null)
                ...
                editor.setValidator(new AttributeValidator(schema.getAttributeType(i), schema));
        }
    }
    ...
    private void createCellEditors() {
        ...
        for( int i = 0; i < schema.getAttributeCount(); i++ ) {
            ...
            Class< ? extends Object> concreteType = aType.getType();
    ...
    private void createAttributeColumns( final Table table, TableViewer viewer, TableLayout layout ) {
        ...
        if (features == null) {
            ...
            for( int i = 0; i < schema.getAttributeCount(); i++ ) {
                ...
                if (Geometry.class.isAssignableFrom(aType.getType())) { // was aType.isGeometry()
                    // jg: wot is this maddness? jd: paul said so
                    column.setText("GEOMETRY"); //$NON-NLS-1$
                } else
                    column.setText(aType.getName());
    ...
    public void select( String cql, boolean selectAll ) throws RuntimeException {
        try {
            ...
            List<Feature> toSearch = provider.features;
            ...
            int j=0;
            int firstMatch=-1;
            OUTER: for( Feature feature : toSearch ) {
    ...
    private boolean searchFeature( Feature feature, Pattern pattern, String[] attributes ) {
