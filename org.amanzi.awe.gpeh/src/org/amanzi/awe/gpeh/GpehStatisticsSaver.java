...
public class GpehStatisticsSaver implements ISaver<GpehTransferData> {
	...
	public void save(GpehTransferData dataElement) {
		...
		Long timestamp = (Long) dataElement
				.get(GpehTransferData.TIMESTAMP_OF_DAY);
		if (timestamp == null) {
			return;
		}

		Object content = dataElement
				.get(Parameters.EVENT_PARAM_MESSAGE_CONTENTS.name());
		if (content != null) {
			if (event.getType() == Events.RRC_MEASUREMENT_REPORT) {
				...
				MessageDecoder.getInstance().parseRRCMeasurementReport(
						messageContent, asnListener);
		...
		if ((Math.abs(fullTime - oldTimestamp) > 15 * 60 * 1000) && !isIMSI) {
			if (oldTimestamp != 0) {
				...
				tx.success();
				tx.finish();
				tx = DatabaseManager.getInstance().getCurrentDatabaseService()
						.beginTx();
			}
			...
			if (Math.abs(fullTime - oldTimestamp) > 3 * 60 * 1000) {
				if (oldTimestamp != 0) {
					...
					tx.success();
					tx.finish();
					tx = DatabaseManager.getInstance()
							.getCurrentDatabaseService().beginTx();
				}
				oldTimestamp = fullTime;
			}
		}
	 ...
	 *            the index info
	 */
	private void processNbapMeasurementReport(GpehTransferData dataElement,
			long timestamp) {
	...
	/**
	 * process RRC mesurement report
	 * 
	 * @param dataElement
	 * @param timestamp
	 */
	private void processRrcMeasurementReport(GpehTransferData dataElement,
			long timestamp) {
		GpehStatisticModel model = getStatisticReport();
	...
	public void init(GpehTransferData element) {
		...
		String datasetNameStat = (String) element.get(GpehTransferData.DATASET)
				+ " Statistic";
		String datasetNameEvent = (String) element
				.get(GpehTransferData.DATASET) + " Event";
		tx = DatabaseManager.getInstance().getCurrentDatabaseService()
				.beginTx();
		Node projectNode = datasetService.findOrCreateAweProject(projectName);
		datasetStatistic = datasetService.getRootNode(projectName,
				datasetNameStat, NodeTypes.DATASET);
		...
		datasetEvent.setProperty(INeoConstants.PROPERTY_NAME_NAME,
				datasetNameEvent);
		...
		datasetStatistic.createRelationshipTo(datasetEvent,
				GeoNeoRelationshipTypes.VIRTUAL_DATASET);
		datasetInfoStatistic = new DatasetInfo(datasetStatistic);
		eventIndexNameStatistic = new StringBuilder("Id")
				.append(datasetStatistic.getId()).append("@")
				.append("gpeh_event").append("@").append("name").toString();
		datasetInfoEvent = new DatasetInfo(datasetEvent);
		eventIndexNameEvent = new StringBuilder("Id")
				.append(datasetEvent.getId()).append("@").append("gpeh_event")
	...
	public synchronized GpehStatisticModel getStatisticReport() {
		if (statmodel == null) {
			if (datasetStatistic.hasRelationship(
					DatasetRelationshipTypes.STATISTICS, Direction.OUTGOING)) {
				Node statNode = datasetStatistic
						.getSingleRelationship(
								DatasetRelationshipTypes.STATISTICS,
								Direction.OUTGOING).getEndNode();
				statmodel = new GpehStatisticModel(datasetStatistic, statNode,
						datasetService.getGraphDatabaseService());
				return new GpehStatisticModel(datasetStatistic, statNode,
						datasetService.getGraphDatabaseService());
			} else {
				// Node statNode = databaseService.createNode();
				// datasetNode.createRelationshipTo(statNode,
				// DatasetRelationshipTypes.GPEH_STATISTICS);
				// return new GpehStatisticModel(datasetNode, statNode,
				// databaseService);
				statmodel = new GpehStatisticModel(datasetStatistic,
						datasetStatistic,
						datasetService.getGraphDatabaseService());
				return statmodel;
			}
	...
	public synchronized GpehStatisticModel getStatisticReportEvent() {
		if (statmodelEvent == null) {
			if (datasetEvent.hasRelationship(
					DatasetRelationshipTypes.STATISTICS, Direction.OUTGOING)) {
				Node statNode = datasetEvent
						.getSingleRelationship(
								DatasetRelationshipTypes.STATISTICS,
								Direction.OUTGOING).getEndNode();
				statmodelEvent = new GpehStatisticModel(datasetEvent, statNode,
						datasetService.getGraphDatabaseService());
				return new GpehStatisticModel(datasetEvent, statNode,
						datasetService.getGraphDatabaseService());
			} else {
				// Node statNode = databaseService.createNode();
				// datasetNode.createRelationshipTo(statNode,
				...
				// return new GpehStatisticModel(datasetNode, statNode,
				// databaseService);
				statmodelEvent = new GpehStatisticModel(datasetEvent,
						datasetEvent, datasetService.getGraphDatabaseService());
	...
	public void finishUp(GpehTransferData element) {
		try {
			...
			datasetStatistic = DatabaseManager.getInstance()
					.getCurrentDatabaseService()
					.getNodeById(datasetStatistic.getId());
			datasetEvent = DatabaseManager.getInstance()
					.getCurrentDatabaseService()
					.getNodeById(datasetEvent.getId());
			try {
				System.out.println(String.format("min_timestamp %s",
						datasetInfoStatistic.getMinTimestamp()));
				datasetStatistic.setProperty("min_timestamp",
						datasetInfoStatistic.getMinTimestamp());
				System.out.println(String.format("max_timestamp %s",
						datasetInfoStatistic.getMaxTimestamp()));
				datasetStatistic.setProperty("max_timestamp",
						datasetInfoStatistic.getMaxTimestamp());

				System.out.println(String.format("min_timestamp %s",
						datasetInfoEvent.getMinTimestamp()));
				datasetEvent.setProperty("min_timestamp",
						datasetInfoEvent.getMinTimestamp());
				System.out.println(String.format("max_timestamp %s",
						datasetInfoEvent.getMaxTimestamp()));
				datasetEvent.setProperty("max_timestamp",
						datasetInfoEvent.getMaxTimestamp());
				tx.success();
			} finally {
				tx.finish();
				...
				if (event.getClassName().equals("UL-DCCH-MessageType")) {
					...
					if (value.contains("InterFreq")) {
						if (!asnDataMap.containsKey(GpehReportUtil.MR_TYPE)) {
							asnDataMap.put(GpehReportUtil.MR_TYPE,
									GpehReportUtil.MR_TYPE_INTERF);
						}
					} else if (value.contains("InterRAT")) {
						if (!asnDataMap.containsKey(GpehReportUtil.MR_TYPE)) {
							asnDataMap.put(GpehReportUtil.MR_TYPE,
									GpehReportUtil.MR_TYPE_IRAT);
						}
						...
						if (!asnDataMap.containsKey(GpehReportUtil.MR_TYPE)) {
							asnDataMap.put(GpehReportUtil.MR_TYPE,
									GpehReportUtil.MR_TYPE_UE_INTERNAL);
						}
					} else {
						return false;
					}
