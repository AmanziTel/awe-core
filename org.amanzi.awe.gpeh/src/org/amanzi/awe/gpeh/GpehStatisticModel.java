...
public class GpehStatisticModel {
	 ...
	 *            the database service
	 */
	public GpehStatisticModel(Node dataset, Node statisticNode,
			GraphDatabaseService databaseService) {
		this.dataset = dataset;
		this.databaseService = databaseService;
	}

	public void processIMSIEvent(HashMap<String, Object> dataElement,
			Long timestamp) {

		Long imsi = (Long) dataElement.get("EVENT_PARAM_IMSI");
		Integer rnc = (Integer) dataElement.get("EVENT_PARAM_RNC_MODULE_ID");
		Integer ue_context = (Integer) dataElement
				.get("EVENT_PARAM_UE_CONTEXT");
		String me_context = (String) dataElement.get("MeContext");
		if (imsi == null) {
			return;
		}
		statType = GpehRelationType.RNC.name();
		processImsi(databaseService, rnc, imsi, timestamp, ue_context,
				me_context);
	 ...
	 *            the data element
	 * @param timestamp
	 *            the actuual timestamp
	 */
	public void processNbapEvent(HashMap<String, Object> dataElement,
			Long timestamp) {
	 ...
	 *            the timestamp
	 */
	private void processHsDlTxRequiredPower(Integer ci, Integer rnc,
			Integer value, Long timestamp) {
		updateNbapStat(ci, rnc, value, timestamp,
				GpehRelationType.HS_DL_TX_RequiredPower);
	}
	 ...
	 *            the timestamp
	 */
	private void processR99DlTxPower(Integer ci, Integer rnc, Integer value,
			Long timestamp) {

		updateNbapStat(ci, rnc, value, timestamp,
				GpehRelationType.R99_DL_TX_POWER);
	}
	 ...
	 *            the timestamp
	 */
	private void processTotalDlTxPower(Integer ci, Integer rnc, Integer value,
			Long timestamp) {
		updateNbapStat(ci, rnc, value, timestamp,
				GpehRelationType.TOTAL_DL_TX_POWER);
	}
	 ...
	 * @return
	 */
	private void processImsi(GraphDatabaseService databaseService, Integer rnc,
			Long imsi, Long timestamp, Integer ue_context, String me_context) {
		updateIMSIStat(databaseService, rnc, imsi, timestamp,
				GpehRelationType.RNC, ue_context, me_context);
	}
	 ...
	 *            the timestamp
	 */
	private void processUlRtwp(Integer ci, Integer rnc, Integer value,
			Long timestamp) {
	 ...
	 * @param dataElement
	 */
	private void updateNbapStat(Integer ci, Integer rnc, Integer value,
			Long timestamp, GpehRelationType rel) {
	...
	/**
	 * IMSI update statistic
	 * 
	 * @param imsi
	 * @param timestamp
	 */
	private void updateIMSIStat(GraphDatabaseService databaseService,
			Integer rnc, Long imsi, Long timestamp, GpehRelationType rel,
	 ...
	 */
	private NBapCache getNBapCache(Node root, int minValue, int maxvalue,
			Long timestamp) {
	 ...
	 * @return
	 */
	private void saveIMSIEvent(GraphDatabaseService databaseService, Node root,
			long beginTimestamp, Long imsi, Integer rnc, Integer ue_context,
			String me_context) {
		...
		if (!virtualParent.hasRelationship(NetworkRelationshipTypes.CHILD,
				Direction.OUTGOING)
				&& NodeTypes.DATASET.checkNode(virtualParent)) {
			virtualCurrentNode = databaseService.createNode();
			virtualParent.createRelationshipTo(virtualCurrentNode,
					NetworkRelationshipTypes.CHILD);

		} else {
			virtualCurrentNode = databaseService.createNode();
			virtualCurrentNode.setProperty("type", NodeTypes.M.getId());
			virtualParent.createRelationshipTo(virtualCurrentNode,
					NetworkRelationshipTypes.NEXT);

		}
		virtualParent = virtualCurrentNode;
	...
	private void saveRNCPool(long beginTimestamp, Node rncNode, Long imsi,
			Integer rnc, Integer ue_context, String me_context) {
		rncNode.setProperty("type", NodeTypes.M.getId());

		rncNode.setProperty("IMSI", imsi);
		rncNode.setProperty("RCN_ID", rnc);
		rncNode.setProperty("ME_CONTEXT", me_context);
		rncNode.setProperty("UE_CONTEXT", ue_context);
	 ...
	 *            the timestamp
	 */
	public void processRrcEvent(HashMap<String, Object> dataElement,
			Long timestamp) {
		@SuppressWarnings("unchecked")
		List<RrcMeasurement> measList = (List<RrcMeasurement>) dataElement
				.remove("MEAS_LIST");
	 ...
	 *            the data element
	 */
	private void handleIUeTxPower(Integer ci, Integer rnc, Long timestamp,
			List<RrcMeasurement> measList, HashMap<String, Object> dataElement) {
	 ...
	 *            the data element
	 */
	private void handleIntraFrMeasurement(Integer ci, Integer rnc,
			Long timestamp, List<RrcMeasurement> measList,
			HashMap<String, Object> dataElement) {
		RrcMeasurement bestCellMeas = measList.get(0);
		if (bestCellMeas.getEcNo() == null || bestCellMeas.getEcNo() > 49
				|| bestCellMeas.getRscp() == null
	 ...
	 *            the data element
	 */
	private void handleInterFrMeasurement(Integer ci, Integer rnc,
			Long timestamp, List<RrcMeasurement> measList,
			HashMap<String, Object> dataElement) {
		RRCCache cache = defineRrcCache(timestamp);
	...
	public static abstract class Cache {
		...
		protected BestCell formBestCell(Node rel) {
			...
			String[] id = rel.getProperty(INeoConstants.PROPERTY_NAME_NAME)
					.toString().split("_");
			try {
				return new BestCell(Integer.valueOf(id[0]),
	...
	public static class NBapCache extends Cache {
		 ...
		 *            the maxvalue
		 */
		public NBapCache(Node root, long beginTimestamp, int minValue,
				int maxvalue) {
		...
		public void load(long startTime, GraphDatabaseService service) {
			...
			if (currentNode != null && checkNode != parentNode) {
				while (checkNode.hasRelationship(Direction.OUTGOING)) {
					if (checkNode.getProperty(STATISTIC_PROPERTY_TYPE).equals(
							statType)
							&& checkNode
									.getProperty(
					...
					if (checkNode.hasRelationship(
							NetworkRelationshipTypes.CHILD, Direction.OUTGOING)) {
						checkNode = checkNode.getSingleRelationship(
								NetworkRelationshipTypes.CHILD,
		...
		protected NbapPool loadPool(long startTime, Node statNode) {
			...
			if (statNode.hasProperty("Range size")
					&& startTime == (Long) statNode.getProperty(
							INeoConstants.PROPERTY_TIMESTAMP_NAME, null)) {
				result.formValues((Integer) statNode.getProperty("Range size"));
				for (int i = 0; i < result.values.length; i++) {
					if (statNode.hasProperty(String
							.format("%s %d", statType, i))) {
						result.values[i] = (Integer) statNode
								.getProperty(String
										.format("%s %d", statType, i));
					}
				}
		...
		public void save(GraphDatabaseService service) {
			for (Map.Entry<BestCell, NbapPool> entry : cellCache.entrySet()) {
				...
				Node result;
				if (!parentNode.hasRelationship(NetworkRelationshipTypes.CHILD,
						Direction.OUTGOING)
						&& NodeTypes.DATASET.checkNode(parentNode)) {

					currentNode = service.createNode();
					currentNode.setProperty("type", NodeTypes.M.getId());
					currentNode.setProperty("name", reltype.name());
					parentNode.createRelationshipTo(currentNode,
							NetworkRelationshipTypes.CHILD);
		 ...
		 *            the nbap pool
		 */
		private void savePool(long beginTimestamp, Node statNode,
				NbapPool nbapPool) {
			if (nbapPool.values != null) {
				statNode.setProperty(INeoConstants.PROPERTY_TIMESTAMP_NAME,
						beginTimestamp);
				for (int i = 0; i < nbapPool.values.length; i++) {
					if (nbapPool.values[i] != 0) {
						statNode.setProperty(
								String.format("%s %d", statType, i),
								nbapPool.values[i]);
						statistic.indexValue(root_key, NodeTypes.M.getId(),
								String.format("%s %d", statType, i),
								nbapPool.values[i]);
	...
	public static class RRCCache extends Cache {
		 ...
		 * @param dataElement
		 */
		public void updateIntraFr(Integer ci, Integer rnc,
				List<RrcMeasurement> measList,
				HashMap<String, Object> dataElement) {
			...
			while (measIter.hasNext()) {
				...
				if (psc == null || sectorMeas.getEcNo() == null
						&& sectorMeas.getRscp() == null) {
					continue;
				}
				IntraPool pul = getIntraPool(cache, psc);
				pul.numMeasurements++;
				if (sectorMeas.getEcNo() != null) {
					double deltaDbm = (double) Math.abs(bestCellMeas.getEcNo()
							- sectorMeas.getEcNo()) / 2;
					if (deltaDbm <= 3) {
						pul.ecnoD[0]++;
					}
					if (deltaDbm <= 6) {
				...
				if (sectorMeas.getRscp() != null) {
					double deltaRscp = (double) Math.abs(bestCellMeas.getRscp()
							- sectorMeas.getRscp()) / 1;
					if (deltaRscp <= 3) {
						pul.rscpD[0]++;
					}
					if (deltaRscp <= 6) {
		 ...
		 * @param dataElement
		 */
		public void updateInterFr(Integer ci, Integer rnc,
				List<RrcMeasurement> measList,
				HashMap<String, Object> dataElement) {
			BestCell cell = new BestCell(ci, rnc);
			Map<String, Pool> cache = getCacheMap(cell);
		 ...
		 */
		private void updateShoStat(RrcBestCellPool pool,
				HashMap<String, Object> dataElement) {
		...
		public void load(long startTime, GraphDatabaseService service) {
			...
			while (startNode.hasRelationship(NetworkRelationshipTypes.CHILD,
				...
				if (cell != null) {
					...
					if (cellRoot.getProperty(INeoConstants.PROPERTY_NAME_NAME)
							.toString().indexOf("_") > -1
							&& cellRoot.getProperty(STATISTIC_PROPERTY_TYPE)
									.equals(statType)) {
		...
		private RrcBestCellPool loadBestCellStat(Node cellRoot) {
			...
			result.numMrForBestCellInter = (Integer) cellRoot.getProperty(
					RRCCache.NUM_INTER_MES_BEST_CELL, 0);
			result.numMrForBestCellIntra = (Integer) cellRoot.getProperty(
					RRCCache.NUM_INTRA_MES_BEST_CELL, 0);
			result.numMrForBestCellIrat = (Integer) cellRoot.getProperty(
					RRCCache.NUM_IRAT_MES_BEST_CELL, 0);
			...
			for (int rscp = 0; rscp <= 91; rscp++) {
				...
				for (int ecno = 0; ecno <= 50; ecno++) {
					if (cellRoot.hasProperty(String.format("RSCP %d ECNO %d",
							rscp, ecno).toString())) {
						rspArr[ecno] = Integer.parseInt(cellRoot.getProperty(
								String.format("RSCP %s ECNO %d", rscp, ecno))
								.toString());
					}
		...
		private void storeBestCellStat(Node cellRoot, RrcBestCellPool cell) {
			...
			statistic.indexValue(root_key, NodeTypes.M.getId(), "oneWay",
					cell.oneWay);
			cellRoot.setProperty("twoWay", cell.twoWay);
			statistic.indexValue(root_key, NodeTypes.M.getId(), "twoWay",
					cell.twoWay);
			cellRoot.setProperty("threeWay", cell.threeWay);
			statistic.indexValue(root_key, NodeTypes.M.getId(), "threeWay",
					cell.threeWay);

			cellRoot.setProperty(NUM_INTER_MES_BEST_CELL,
					cell.numMrForBestCellInter);
			statistic.indexValue(root_key, NodeTypes.M.getId(),
					NUM_INTER_MES_BEST_CELL, cell.numMrForBestCellInter);
			cellRoot.setProperty(NUM_INTRA_MES_BEST_CELL,
					cell.numMrForBestCellIntra);
			statistic.indexValue(root_key, NodeTypes.M.getId(),
					NUM_INTRA_MES_BEST_CELL, cell.numMrForBestCellIntra);
			cellRoot.setProperty(NUM_IRAT_MES_BEST_CELL,
					cell.numMrForBestCellIrat);
			statistic.indexValue(root_key, NodeTypes.M.getId(),
					NUM_IRAT_MES_BEST_CELL, cell.numMrForBestCellIrat);

			cellRoot.setProperty(INeoConstants.PROPERTY_TIMESTAMP_NAME,
					beginTimestamp);
			statistic.indexValue(root_key, NodeTypes.M.getId(),
					INeoConstants.PROPERTY_TIMESTAMP_NAME, beginTimestamp);
			...
			if (cell.rscpEcno != null) {
				...
				for (int rscp = 0; rscp <= 91; rscp++) {
					...
					for (int index = 0; index < ecnoArr.length; index++) {
						...
						if (ecnoArr[index] > 0) {
							cellRoot.setProperty(String.format(
									"RSCP %d ECNO %d", rscp, index),
									ecnoArr[index]);
						}
					}
				}
		...
		private void storePool(Node node, Pool pool) {
			if (pool.interPool != null) {
				...
				statistic.indexValue(root_key, NodeTypes.M.getId(),
						NUM_INTER_MES, pool.interPool.numMeasurements);
				for (int i = 0; i < paramInterEcnoList.size(); i++) {
					node.setProperty(paramInterEcnoList.get(i),
							pool.interPool.ecno[i]);
					statistic.indexValue(root_key, NodeTypes.M.getId(),
							paramInterEcnoList.get(i), pool.interPool.ecno[i]);

				}
				for (int i = 0; i < (paramInterRSCPECNO14.size() + paramInterRSCPECNO10
						.size()); i++) {
					if (i < paramInterRSCPECNO14.size()) {
						node.setProperty(paramInterRSCPECNO14.get(i),
								pool.interPool.rscp[i]);
						statistic.indexValue(root_key, NodeTypes.M.getId(),
								paramInterRSCPECNO14.get(i),
								pool.interPool.rscp[i]);
					} else {
						node.setProperty(
								paramInterRSCPECNO10.get(i
										- paramInterRSCPECNO10.size()),
								pool.interPool.rscp[i]);
						statistic.indexValue(
								root_key,
								NodeTypes.M.getId(),
								paramInterRSCPECNO10.get(i
										- paramInterRSCPECNO10.size()),
				...
				node.setProperty(INeoConstants.PROPERTY_TIMESTAMP_NAME,
						beginTimestamp);
				statistic.indexValue(root_key, NodeTypes.M.getId(),
						INeoConstants.PROPERTY_TIMESTAMP_NAME, beginTimestamp);
			}
			if (pool.intraPool != null) {
				...
				statistic.indexValue(root_key, NodeTypes.M.getId(),
						NUM_INTER_MES, pool.intraPool.numMeasurements);
				for (int i = 0; i < paramIntraEcnoListDelta.size(); i++) {
					node.setProperty(paramIntraEcnoListDelta.get(i),
							pool.intraPool.ecnoD[i]);
					statistic.indexValue(root_key, NodeTypes.M.getId(),
							paramIntraEcnoListDelta.get(i),
							pool.intraPool.ecnoD[i]);
				}
				for (int i = 0; i < paramIntraRSCPListDelta.size(); i++) {
					node.setProperty(paramIntraRSCPListDelta.get(i),
							pool.intraPool.rscpD[i]);
					statistic.indexValue(root_key, NodeTypes.M.getId(),
							paramIntraRSCPListDelta.get(i),
							pool.intraPool.rscpD[i]);
				}
				for (int i = 0; i < paramPositionList.size(); i++) {
					node.setProperty(paramPositionList.get(i),
							pool.intraPool.position[i]);
					statistic.indexValue(root_key, NodeTypes.M.getId(),
							paramPositionList.get(i),
							pool.intraPool.position[i]);
				}
		...
		private Pool loadPoolFromNode(Node node) {
			...
			if (node.hasProperty(NUM_INTER_MES)) {
				...
				inter.numMeasurements = (Integer) node
						.getProperty(NUM_INTER_MES);
				for (int i = 0; i < paramInterEcnoList.size(); i++) {
					if (node.hasProperty(paramInterEcnoList.get(i))) {
						inter.ecno[i] = (Integer) node
								.getProperty(paramInterEcnoList.get(i));
					}
				...
				for (int i = 0; i < (paramInterRSCPECNO14.size() + paramInterRSCPECNO10
					...
					if (i < paramInterRSCPECNO14.size()) {
						inter.rscp[i] = (Integer) node
								.getProperty(paramInterRSCPECNO14.get(i));
					} else {
						inter.rscp[i] = (Integer) node
								.getProperty(paramInterRSCPECNO10.get(i
										- paramInterRSCPECNO10.size()));
					}
				}
			...
			if (node.hasProperty(NUM_INTRA_MES)) {
				...
				intra.numMeasurements = (Integer) node
						.getProperty(NUM_INTRA_MES);
				for (int i = 0; i < paramIntraEcnoListDelta.size(); i++) {
					if (node.hasProperty(paramIntraEcnoListDelta.get(i))) {
						intra.ecnoD[i] = (Integer) node
								.getProperty(paramIntraEcnoListDelta.get(i));
					}
				}

				for (int i = 0; i < paramIntraRSCPListDelta.size(); i++) {
					if (node.hasProperty(paramIntraRSCPListDelta.get(i))) {
						intra.rscpD[i] = (Integer) node
								.getProperty(paramIntraRSCPListDelta.get(i));
					}
				}

				for (int i = 0; i < paramPositionList.size(); i++) {
					if (node.hasProperty(paramPositionList.get(i))) {
						intra.position[i] = (Integer) node
								.getProperty(paramPositionList.get(i));
					}
				}
		...
		public void save(GraphDatabaseService service) {
			...
			for (Map.Entry<BestCell, RrcBestCellPool> entry : mainCache
					.entrySet()) {
				if (!parentNode.hasRelationship(NetworkRelationshipTypes.CHILD,
						Direction.OUTGOING)
						&& NodeTypes.DATASET.checkNode(parentNode)) {
					currentNode = service.createNode();
					currentNode.setProperty("type", NodeTypes.M.getId());
					currentNode.setProperty("name", entry.getKey().getCi()
							+ "_" + entry.getKey().getRnc());
				...
				currentNode
						.setProperty(Cache.STATISTIC_PROPERTY_TYPE, statType);
				statistic.indexValue(root_key, NodeTypes.M.getId(),
						STATISTIC_PROPERTY_TYPE, statType);
				statistic.increaseTypeCount(root_key, NodeTypes.M.getId(), 1);
				...
				for (Map.Entry<String, Pool> entryCache : entry.getValue().cache
					...
					try {
						if (!parentNode.hasRelationship(Direction.OUTGOING)) {
							currentNode = service.createNode();
							currentNode
									.setProperty("type", NodeTypes.M.getId());
							currentNode.setProperty("name", relt.name());
							parentNode.createRelationshipTo(currentNode,
									NetworkRelationshipTypes.NEXT);
						} else {
							currentNode = service.createNode();
						...
						currentNode.setProperty(STATISTIC_PROPERTY_TYPE,
								statType);
						statistic.indexValue(root_key, NodeTypes.M.getId(),
								STATISTIC_PROPERTY_TYPE, statType);
