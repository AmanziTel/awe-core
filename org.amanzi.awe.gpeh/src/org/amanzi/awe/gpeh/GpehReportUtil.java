...
public class GpehReportUtil {
	...
	public static class MatrixProperties {
		...
		public static String getRSCPECNOPropertyName(int prfx, int ecnodmn) {
			return new StringBuilder("RSCP").append(prfx).append("_")
					.append(ecnodmn).toString();
		}
	}
	...
	public static boolean isReportProperties(String key) {
		return isTxPowerProperty(key) || isECNOProperty(key)
				|| isRSCPProperty(key);
	}
	...
	public static enum ReportsRelations implements RelationshipType {
		...
		CELL_RSCP_ANALYSYS,
		/** The SOURCE event. */
		SOURCE_EVENT, SOURCE_RSCP_EVENT, SOURCE_ECNO_EVENT, SOURCE_RSCP_ECNO_EVENT, SOURCE_UE_TX_POWER_EVENT, SOURCE, SOURCE_DE_TX_CODE_POWER, SOURCE_UL_INTERFERENCE, SOURCE_DL_TX_CARRIER_POWER, SOURCE_NON_HS_POWER, SOURCE_HSDSCH_REQUIRED_POWER;
	}
	 ...
	 * @param networkName
	 *            the network name
	 * @param gpehEventsName
	 *            the gpeh events name
	 * @param type
	 *            the type
	 * @return the matrix lucene index name
	 */
	public static String getMatrixLuceneIndexName(String networkName,
			String gpehEventsName, String type) {
		return getReportId(networkName, gpehEventsName).append("@")
	 ...
	 */
	public static StringBuilder getReportId(String networkName,
			String gpehEventsName) {
		return new StringBuilder(networkName).append("@")
				.append(gpehEventsName);
	}
	 ...
	 * @return the integer
	 */
	public static Integer findPrefix(Node gpehEvent, String psc,
			GraphDatabaseService service) {
		Transaction tx = service.beginTx();
		try {
			int i = 0;
			String psc2;
			while ((psc2 = (String) gpehEvent.getProperty(
					GpehReportUtil.GPEH_RRC_SCRAMBLING_PREFIX + (++i), null)) != null) {
				if (psc.equals(psc2)) {
					return i;
				}
			}
