...
public class NewNetworkSaverTesting extends AbstractAWETest {
	private static Logger LOGGER = Logger
			.getLogger(NewNetworkSaverTesting.class);
	...
	public static void tearDownAfterClass() throws Exception {
		stopDb();
		clearDb();
		LOGGER.info("NewNetworkSaverTesting finished in "
	...
	public void onStart() throws AWEException {
		...
		networkSaver = new NewNetworkSaver(model,
				(ConfigurationDataImpl) config, service);
	...
	public void testForSavingAllElements() {
		...
		try {
			...
			when(model.createElement(any(IDataElement.class), eq(BSC)))
					.thenReturn(new DataElement(BSC));
			when(model.findElement(SITE)).thenReturn(null);
			when(model.createElement(any(IDataElement.class), eq(SITE)))
					.thenReturn(new DataElement(SITE));
			when(model.findElement(SECTOR)).thenReturn(null);
			when(model.createElement(any(IDataElement.class), eq(SECTOR)))
					.thenReturn(new DataElement(SECTOR));
			when(model.findElement(MSC)).thenReturn(null);
			when(model.createElement(any(IDataElement.class), eq(MSC)))
					.thenReturn(new DataElement(MSC));
			when(model.findElement(CITY)).thenReturn(null);
			when(model.createElement(any(IDataElement.class), eq(CITY)))
					.thenReturn(new DataElement(CITY));
			networkSaver.saveElement(rowContainer);
			verify(model, times(5)).createElement(any(IDataElement.class),
					any(Map.class));
		} catch (Exception e) {
	...
	public void testForSavingSITESECTOR() {
		...
		try {
			...
			when(model.createElement(any(IDataElement.class), eq(SECTOR)))
					.thenReturn(new DataElement(SECTOR));
			networkSaver.saveElement(rowContainer);
			verify(model, times(2)).createElement(any(IDataElement.class),
					any(Map.class));
		} catch (Exception e) {
	...
	public void testForTyingToSaveOnlySector() {
		...
		try {
			networkSaver.saveElement(rowContainer);
			verify(model, never()).createElement(any(IDataElement.class),
					any(Map.class));
		} catch (Exception e) {
	...
	public void testForTyingToSaveSectorAndSiteWithLatAndLon() {
		...
		try {
			...
			LOGGER.error(" testForTyingToSaveSectorAndSiteWithLatAndLon error",
					e);
			Assert.fail("Exception while saving row");
		}
	}
	...
	public void testTransactionRollBackIfDatabaseExceptionThrow() {
		...
		try {
			rowContainer.setValues(values);
			when(model.findElement(any(Map.class))).thenThrow(
					new DatabaseException("required exception"));
			// when(model.createElement(any(IDataElement.class),
			// eq(SITE))).thenReturn(new
			// DataElement(SITE));
			// when(model.findElement(SECTOR)).thenReturn(null);
			// when(model.createElement(any(IDataElement.class),
			// eq(SECTOR))).thenReturn(new
			// DataElement(SECTOR));
			networkSaver.saveElement(rowContainer);
		} catch (Exception e) {
	...
	public void testTransactionRollBackIfRestExceptionThrow() {
		...
		try {
			rowContainer.setValues(values);
			when(model.findElement(any(Map.class))).thenThrow(
					new IllegalArgumentException("required exception"));
			// when(model.createElement(any(IDataElement.class),
			// eq(SITE))).thenReturn(new
			// DataElement(SITE));
			// when(model.findElement(SECTOR)).thenReturn(null);
