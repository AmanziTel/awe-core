...
def detect_rspec_examples?
  Object.const_defined?("Spec") && rspec_options_override.instance_variable_get("@example_groups").first
end
...
if detect_rspec_examples?
  require File.join(File.dirname(__FILE__), "RemoteTestRunnerRSpec.rb")
  rspec_options_override.user_input_for_runner = "Spec::Runner::EclipseExampleGroupRunner"
  rspec_options_override.formatters.clear
  rspec_options_override.formatters << Spec::Runner::Formatter::EclipseProgressBarFormatter.new(rspec_options_override, session || STDOUT)
  rspec_options_override.files.delete_if{|f| true }
  Spec.run
