...
public class RefreshGemsActionDelegate implements IObjectActionDelegate, IViewActionDelegate
{
	...
	public void run(IAction action)
	{
		UIJob job = new UIJob("Refreshing gem listing")
		{
			...
			{
				if (!getGemManager().refresh())
				{
					MessageDialog.openError(Display.getCurrent().getActiveShell(), "Unable to refresh local gems",
							"Sorry we were unable to refresh the list of local gems.");
				}
