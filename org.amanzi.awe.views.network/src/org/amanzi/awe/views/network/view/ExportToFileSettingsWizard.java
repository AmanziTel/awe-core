...
public class ExportToFileSettingsWizard extends Wizard {
	...
	public ExportToFileSettingsWizard(INetworkModel network) {
		...
		setWindowTitle(NetworkMessages.EXPORT_WINDOW_TITLE);
	...
	protected void export(IProgressMonitor monitor) {
		...
		// get all sectors
		List<IDataElement> sectors = Lists.newArrayList(network
		...
		Comparator<IDataElement> comp = new Comparator<IDataElement>() {
			...
			public int compare(IDataElement arg0, IDataElement arg1) {
				return (arg0.get(AbstractService.NAME).toString())
						.compareTo(arg1.get(AbstractService.NAME).toString());
			};
		};
		...
		for (IDataElement sector : sectors) {
			...
			for (String sectorProperty : sectorProperties) {
				if (!allProperties.contains(sectorProperty)) {
					String s;
					try {
						s = esm.getExportHeader(network,
								NetworkElementNodeType.SECTOR, sectorProperty);
					} catch (DatabaseException e) {
						MessageDialog.openError(null,
								NetworkMessages.ERROR_TITLE, e.getMessage());
	 ...
	 * @throws IOException
	 */
	private void writeRow(FileWriter writer, List<String> values)
			throws IOException {
