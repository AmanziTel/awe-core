            ...
            public void selectionChanged(SelectionChangedEvent event) {
                if (notInterruptEvent) {
                    ...
                    if (model != null) {
                        ...
                        model.setSelectedDataElements(Arrays.asList(selectedDataElements.toArray()));
                        eventManager.fireEvent(new ShowOnMapEvent(model, false));
                    }
    ...
    private void updateNetworkPropertiesView(boolean isEditable) {
        try {
            ...
            NetworkPropertiesView propertiesView = (NetworkPropertiesView)PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                    .getActivePage().showView(NETWORK_PROPERTIES_VIEW_ID);
            propertiesView.updateTableView(selectedDataElements, isEditable);
        } catch (PartInitException e) {
            MessageDialog.openError(null, ERROR_TITLE, "Can't open Network Properties View for loading IDataElement: " + e);
        }
    }
