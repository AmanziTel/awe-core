...
public class CopyOfElementDialog extends AbstractDialog<Integer> {
	...
	public CopyOfElementDialog(Shell parent, IDataElement element,
	...
	private void updateProperies() {
		...
		for (String property : element.keySet()) {
			if (!properties.contains(property)) {
				if (!property.equals(NetworkService.SECTOR_COUNT)
						&& !property.equals(AbstractService.TYPE)) {
	...
	private boolean saveElement() throws AWEException {
		...
		// network
		INetworkModel networkModel = (INetworkModel) element
		...
		if (params.get(AbstractService.NAME).toString().isEmpty()) {
			MessageDialog.openError(shell, NetworkMessages.NAME_ERROR_TITLE,
					NetworkMessages.NAME_ERROR);
			return false;
		} else if (networkModel.findElementByPropertyValue(typeNode,
				AbstractService.NAME, params.get(AbstractService.NAME)).size() != 0) {
		...
		// create element
		networkModel.createElement(networkModel.getParentElement(element),
				params);
		return true;
	}
		...
		public void inputChanged(Viewer viewer, Object oldInput, Object newInput) {
			...
			if (element != null) {
				for (String property : properties) {
					...
					// other
					if (typeElement == 0) {
						elementProperties = new ArrayList<String>(
								Arrays.asList(OTHER_PROPERTIES));
					}

					// site
					if (typeElement == 1) {
						elementProperties = new ArrayList<String>(
								Arrays.asList(SITE_PROPERTIES));
					}

					// sector
					if (typeElement == 2) {
						elementProperties = new ArrayList<String>(
								Arrays.asList(SECTOR_PROPERTIES));
					}
