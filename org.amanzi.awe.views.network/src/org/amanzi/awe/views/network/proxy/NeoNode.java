...
public class NeoNode {
	...
	public NeoNode(Node node, int number) {
		this.node = node;
		if (node.getProperty(INeoConstants.PROPERTY_TYPE_NAME, "").equals(
				NodeTypes.FREQUENCY_PLAN.getId())) {
			String theName;
			theName = NeoUtils.getFormatedNodeName(
					node.getSingleRelationship(DatasetRelationshipTypes.CHILD,
							Direction.INCOMING).getStartNode(), "");

			Object propertyValue = node.getProperty("arfcn", null);
			if (propertyValue != null)
				theName = theName + ", ARFCN=" + propertyValue.toString();
			propertyValue = node.getProperty("bsic", null);
			if (propertyValue != null)
				theName = theName + ", BSIC=" + propertyValue.toString();

			propertyValue = node.getProperty("hsn", null);
			if (propertyValue != null)
				theName = theName + ", HSN=" + propertyValue.toString();

			propertyValue = node.getProperty("maio", null);
			if (propertyValue != null)
				theName = theName + ", MAIO=" + propertyValue.toString();
			this.name = theName;
	...
	public NeoNode[] getChildren() {
		...
		if (children == null) {
			...
			childrens = node.traverse(Order.BREADTH_FIRST,
			...
			/*
			 * while (childrens.hasNext()){ System.out.println("1: " +
			 * childrens.next().toString()); }
			 */
			...
			if (children.size() == 0) {
				for (Relationship relationship : node.getRelationships(
						NetworkRelationshipTypes.MISSING, Direction.OUTGOING)) {
	...
	public String getType() {
		return (String) node
				.getProperty(INeoConstants.PROPERTY_TYPE_NAME, null);
	}
	...
	public static class NeoNodeComparator implements Comparator<NeoNode> {
		...
		public int compare(NeoNode o1, NeoNode o2) {
			return o1 == null ? -1 : o2 == null ? 1 : o1.name
					.compareTo(o2.name);
		}
	...
	public void setName(String value) {
		Transaction tx = NeoServiceProviderUi.getProvider().getService()
				.beginTx();
		try {
			...
			if (node.hasProperty(INeoConstants.PROPERTY_NAME_NAME)) {
				...
				name = value;
				node.setProperty(INeoConstants.PROPERTY_OLD_NAME,
						oldName.toString());
				tx.success();
