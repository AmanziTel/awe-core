...
/**
 * <p>
 * Test for call statistics from XML files (Group calls)
 * </p>
 * @author Shcharbatsevich_A
 * @since 1.0.0
 */
public class GroupCallXmlStatTest extends GroupCallsStatTest{
    ...
    protected IDataGenerator getDataGenerator(Integer aHours, Integer aDrift, Integer aCallsPerHour,
        ...
        IDataGenerator generator = DataGenerateManager.getXmlGroupAmsGenerator(dataDir, aHours,aDrift, aCallsPerHour, aCallPerHourVariance, aProbes, getMaxGroupSize());
        return generator;
    }
    ...
    protected HashMap<IStatisticsHeader, Number> getAudioDelayStatistics(Call call) {
        ...
        result.put(StatisticsHeaders.GR_DELAY_COUNT_P1,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_P1,getSumFromList(list));
        list = delayMap.get(1);
        result.put(StatisticsHeaders.GR_DELAY_COUNT_P2,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_P2,getSumFromList(list));
        list = delayMap.get(2);
        result.put(StatisticsHeaders.GR_DELAY_COUNT_P3,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_P3,getSumFromList(list));
        list = delayMap.get(3);
        result.put(StatisticsHeaders.GR_DELAY_COUNT_P4,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_P4,getSumFromList(list));
        list = delayMap.get(4);
        result.put(StatisticsHeaders.GR_DELAY_COUNT_L1,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_L1,getSumFromList(list));
        list = delayMap.get(5);
        result.put(StatisticsHeaders.GR_DELAY_COUNT_L2,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_L2,getSumFromList(list));
        list = delayMap.get(6);
        result.put(StatisticsHeaders.GR_DELAY_COUNT_L3,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_L3,getSumFromList(list));
        list = delayMap.get(7);
        result.put(StatisticsHeaders.GR_DELAY_COUNT_L4,list==null||list.isEmpty()?null:list.size());
        result.put(StatisticsHeaders.GR_DELAY_Z1_L4,getSumFromList(list));
        
        result.put(StatisticsHeaders.GR_DELAY_MIN,delays.get(0));
        result.put(StatisticsHeaders.GR_DELAY_MAX,delays.get(delays.size()-1));
        result.put(StatisticsHeaders.GR_DELAY_TOTAL,getSumFromList(delays));
        return result; 
    }
