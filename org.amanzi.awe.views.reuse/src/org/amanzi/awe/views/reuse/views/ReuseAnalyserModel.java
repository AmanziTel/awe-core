...
public class ReuseAnalyserModel {
    ...
    public Node findOrCreateAggregateNode(Node gisNode, final String propertyName, boolean isStringProperty, final String distribute, final String select, IProgressMonitor monitor) {
        ...
        try {
            ...
            if (iterator.hasNext()) {
                ...
                // if (distributeColumn != Distribute.AUTO &&
                // result.hasProperty(INeoConstants.PROPERTY_CHART_ERROR_NAME)
                // && (Boolean)result.getProperty(INeoConstants.PROPERTY_CHART_ERROR_NAME)) {
                // // ActionUtil.getInstance().runTask(new Runnable() {
                // //
                // // @Override
                // // public void run() {
                // // MessageDialog.openError(Display.getCurrent().getActiveShell(), ERROR_TITLE,
                // ERROR_MSG);
                // // }
                // // }, true);
                ...
                // Distribute.AUTO.toString(), select, monitor);
                // }
                return result;
            }
     ...
     */
    private boolean computeStatistics(Node gisNode, Node aggrNode, final String propertyName, boolean isStringProperty, Distribute distribute, Select select,
        ...
        if (isAggregatedProperty) {
            ...
            for (Path path : travers) {
                ...
                if (node.hasProperty(propertyName)) {
                    ...
                    LOGGER.debug("No such property '" + propertyName + "' for node " + (node.hasProperty("name") ? node.getProperty("name").toString() : node.toString()));
                }
                monitor.worked(1);
                if (monitor.isCanceled())
                    break;
    ...
    private boolean computeTransmissionStatistics(Node neighbour, Node aggrNode, String propertyName, Distribute distribute, Select select, IProgressMonitor monitor) {
        ...
        monitor.beginTask("Calculating statistics for " + propertyName, totalWork);
        TreeMap<Column, Integer> result = new TreeMap<Column, Integer>();
        TraversalDescription travDescription = Traversal.description().depthFirst().uniqueness(Uniqueness.NONE).relationships(NetworkRelationshipTypes.CHILD, Direction.OUTGOING)
        ...
        for (Path path : travers) {
            ...
            Double value = null;
            for (Column column : keySet) {
                value = value == null || select == Select.EXISTS ? getTransmissionValue(node, transmissionName, propertyName, select, column.getMinValue(), column.getRange())
                        : value;
                if (value != null && column.containsValue(value)) {
                    Integer count = result.get(column);
                    column.getNode().createRelationshipTo(node, NetworkRelationshipTypes.AGGREGATE);
    ...
    public Node findOrCreateAggregateNode(ISelectionInformation information, final String propertyName, final Object distribute, final String select, IProgressMonitor monitor) {
        ...
        try {
            ...
            try {
                ...
                rootNode.createRelationshipTo(result, NetworkRelationshipTypes.AGGREGATION);
                Iterator<Relationship> links = rootNode.getRelationships(CorrelationRelationshipTypes.CORRELATED, Direction.OUTGOING).iterator();
                while (links.hasNext()) {
                    Relationship link = links.next();
                    Node network = link.getEndNode().getSingleRelationship(CorrelationRelationshipTypes.CORRELATION, Direction.INCOMING).getStartNode();
                    Node gis = NeoUtils.getGisNodeByDataset(network);
    ...
    private void computeStatistics(ISelectionInformation information, Node rootNode, Node aggrNode, String propertyName, Distribute distribute, Select rules,
        ...
        if (Number.class.isAssignableFrom(type)) {
            Double min = ((Number)inf.getStatistic().getMin()).doubleValue();
            Double max = ((Number)inf.getStatistic().getMax()).doubleValue();;
            double range = 0;
            if (min == null || max == null) {
                // error calculation
                throw new StatisticCalculationException(ERROR_MSG_NULL);
            ...
            Map<Object, Long> valueMap = inf.getStatistic().getValueMap();
            Node parent = aggrNode;
            for (Object property : valueMap.keySet()) {
                Column column = new Column(aggrNode, parent, 0, 0.0, distribute, String.valueOf(property), service);
