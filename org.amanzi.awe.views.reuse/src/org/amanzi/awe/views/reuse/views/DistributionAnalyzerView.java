...
public class DistributionAnalyzerView extends ViewPart {
	...
	private class DistributionDataset extends AbstractDataset implements
			CategoryDataset {
		...
		public DistributionDataset() {
			ROW_KEYS.add(Messages.VALUES_AXIS_NAME);
		}
		...
		public Comparable getRowKey(int arg0) {
			return Messages.VALUES_AXIS_NAME;
		}
	...
	private final Job UPDATE_BAR_COLORS_JOB = new Job(StringUtils.EMPTY) {
		...
		protected IStatus run(IProgressMonitor monitor) {
			try {
				...
				return new Status(Status.ERROR, ReusePlugin.PLUGIN_ID,
						Messages.UPDATE_BARS_ERROR, e);
			}
	...
	private class ChangeColorListener implements SelectionListener {
		...
		public void widgetSelected(SelectionEvent e) {
			distributionModel.setLeftColor(convertToColor(leftColor
					.getColorValue()));
			distributionModel.setRightColor(convertToColor(rightColor
					.getColorValue()));
	...
	private void createXmlFileDialog() {
		...
		xmlFileDialog.setText(Messages.SELECT_XML_DIALOG_LABEL);
	...
	private void initMenuManager() {
		...
		actLoadXml = new Action(Messages.LOAD_XML_LABEL) {
			@Override
			public void run() {
				String selected = xmlFileDialog.open();
				try {
					DistributionManager.getManager()
							.createDistributionFromFile(selected);
				} catch (Exception e) {
					showErrorMessage(e.getMessage());
				}
			}
	...
	private void createDistributionChart(Composite parent) {
		...
		distributionChart = ChartFactory.createBarChart(
				Messages.DISTRIBUTION_CHART_NAME, Messages.VALUES_AXIS_NAME, Messages.NUMBERS_AXIS_NAME,
	...
	/**
	 * Creates Combos to choose Distribution (Dataset, Property Name,
	 * Distribution Type)
	 * 
	 * @param parent
	 */
	private void createDistributionSelectionCombos(Composite parent) {
		...
		datasetNameLabel.setText(Messages.DATASET_LABEL);
		...
		propertyNameLabel.setText(Messages.PROPERTY_LABEL);
		...
		distributionTypeLabel.setText(Messages.DISTRIBUTION_LABEL);
		...
		selectLabel.setText(Messages.SELECT_LABEL);
	...
	private void createColoringPropertiesControl(Composite parent) {
		...
		colorPropertiesLabel.setText(Messages.COLOR_PROPERTIES_LABEL);
		...
		chartTypeLabel.setText(Messages.CHART_TYPE_LABEL);
		...
		selectedValueLabel.setText(Messages.SELECTED_VALUES_LABEL);
		...
		selectionAdjacencyLabel.setText(Messages.SELECTION_ADJACENCY_LABEL);
		...
		blendLabel.setText(Messages.BLEND_LABEL);
		...
		leftColor.getButton().setToolTipText(Messages.LEFT_COLOR_LABEL);
		rightColor = new ColorEditor(parent);
		rightColor.getButton().setToolTipText(Messages.RIGHT_COLOR_LABEL);
		...
		paletteLabel.setText(Messages.PALETTE_LABEL);
		...
		thirdColorLabel.setText(Messages.THIRD_COLOR_LABEL);
		...
		middleColor.getButton().setToolTipText(Messages.MIDDLE_COLOR_LABEL);
	...
	private void setBlendPanelVisible(boolean isVisible) {
		...
		if (isVisible) {
			leftColor.setColorValue(convertToRGB(distributionModel
					.getLeftColor()));
			rightColor.setColorValue(convertToRGB(distributionModel
					.getRightColor()));
	...
	private void setThirdColorPanelVisible(boolean isVisible) {
		...
		if (isVisible) {
			middleColor.setColorValue(convertToRGB(distributionModel
					.getMiddleColor()));
		}
	}
	...
	private void updateDistributionsIteams() {
		...
		try {
			for (DistributionItem singleItem : currentProject
					.getAllDistributionalModels()) {
				distributionItems.put(singleItem.toString(), singleItem);
			}
			datasetCombo.setItems(distributionItems.keySet().toArray(
	...
	private void initializePropertyList() {
		...
		if (!StringUtils.isEmpty(itemName)) {
			...
			String[] propertyNames = analyzedModel
					.getAllPropertyNames(analyzedNodeType);
	...
	/**
	 * Initialized list of Distribution Types available for selected property
	 * and data
	 */
	private void initializeDistributionCombo() {
		try {
			...
			if (!StringUtils.isEmpty(propertyName)) {
				List<IDistribution<?>> distribuitons = DistributionManager
						.getManager().getDistributions(analyzedModel,
								analyzedNodeType, propertyName,
								ChartType.getDefault());

				distributionTypes.clear();
				for (IDistribution<?> singleDistribution : distribuitons) {
					distributionTypes.put(singleDistribution.getName(),
							singleDistribution);
				}

				String[] distributionItems = distributionTypes.keySet()
				...
				for (ChartType chartType : DistributionManager.getManager()
						.getPossibleChartTypes(analyzedModel, analyzedNodeType,
								propertyName)) {
					chartTypeNames = (String[]) ArrayUtils.add(chartTypeNames,
							chartType.getTitle());
	...
	private void initializeDistributionType() {
		...
		if (!StringUtils.isEmpty(distribution)) {
			...
			// fill Select combo
			Select[] possibleSelects = currentDistributionType
					.getPossibleSelects();
	...
	private void enableColorPropertiesButton() {
		...
		if (!StringUtils.isEmpty(distribution)) {
			...
			colorPropertiesButton.setEnabled(currentDistributionType
					.canChangeColors());
			colorPropertiesButton.setVisible(currentDistributionType
					.canChangeColors());
		}
	}
	...
	private void runAnalyzis() {
		...
		Job distributionJob = new Job("Create Distribution model <"
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				try {
					if (distributionModel != null) {
						// if old distribution model exists - mark it as
						// not-current
						distributionModel.setCurrent(false);
						distributionModel.finishUp();
					}

					distributionModel = analyzedModel
							.getDistributionModel(currentDistributionType);
					...
					showErrorMessage(e.getMessage());
					return new Status(IStatus.ERROR, ReusePlugin.PLUGIN_ID,
							getName(), e);
				}
		...
		chartFrame.addChartMouseListener(new ChartMouseListener() {
			...
			public void chartMouseClicked(ChartMouseEvent arg0) {
				...
				if (entity instanceof CategoryItemEntity) {
					...
					IDistributionBar newSelectedBar = (IDistributionBar) itemEntity
							.getColumnKey();

					if (selectedBar != null
							&& newSelectedBar.equals(selectedBar)) {
						needRedraw = false;
					} else {
				...
				// TODO: also it should open NetworkTreeView with this
				// Distribution
			}
		});
	...
	private void updateChartColors() {
		...
		float percMid2 = size - midColumnIndex == 0 ? 1 : (float) 1
				/ (size - midColumnIndex);

		Color[] paletteColors = null;
		if (currentPalette != null) {
			paletteColors = currentPalette.getColors(currentPalette
					.getMaxColors());
		}

		for (int i = 0; i < dataset.getDistributionBars().size(); i++) {
			Color barColor = null;
			IDistributionBar currentBar = (IDistributionBar) dataset
					.getColumnKey(i);
			if (currentDistributionType.canChangeColors()) {
				if (colorPropertiesButton.getSelection()) {
					...
					if (selectedIndex >= 0) {
						...
						} else if (Math.abs(selectedIndex - i) <= selectionAdjacencySpin
								.getSelection()) {
							barColor = i > selectedIndex ? COLOR_MORE
									: COLOR_LESS;
						}
					}
				}
	...
	private Color blend(RGB bg, RGB fg, float factor) {
		...
		RGB rgb = new RGB((int) (complement * bg.red + factor * fg.red),
	...
	private void updatePalette() {
		if (paletteCombo.getSelectionIndex() >= 0) {
			...
			currentPalette = PlatformGIS.getColorBrewer().getPalette(
					paletteName);
		} else {
			currentPalette = null;
		}
