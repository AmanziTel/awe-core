...
public class DistributionServiceTest extends AbstractNeoServiceTest {
	...
	private static final String[] USER_DEFINED_DISTR_NAMES = new String[] {
	...
	public void setUp() throws Exception {
		distributionService = NeoServiceFactory.getInstance()
	...
	public void tryToFindRootAggregationNodesWithoutParentNode()
			throws Exception {
	...
	public void checkFindRootAggregationNodesCount() throws Exception {
		...
		List<Node> res = distributionService
				.findRootAggregationNodes(parentNode);

		assertEquals("Search found incorrect count of nodes",
				NUMBER_OF_ROOT_AGGREGATIONS, res.size());
	}
	...
	public void checkFindRootAggregationNodes() throws Exception {
		...
		for (int i = 0; i < NUMBER_OF_ROOT_AGGREGATIONS; i++) {
			nodes.add(createRootAggregationNode(parentNode, DISTRIBUTION_NAME
					+ i));
		}
	...
	@Test(expected = IllegalArgumentException.class)
	public void tryToFindRootAggregationNodeWithoutParentNode()
			throws Exception {
	...
	public void checkSingleResultOfSearch() throws Exception {
		Node parentNode = getParentNode();
		Node rootAggregation = createRootAggregationNode(parentNode,
				DISTRIBUTION_NAME);

		Node result = distributionService.findRootAggregationNode(parentNode,
				distribution);
	...
	public void checkObjectNotExists() throws Exception {
		Node parentNode = getParentNode();
		createRootAggregationNode(parentNode, DISTRIBUTION_NAME
				+ DISTRIBUTION_NAME);
	...
	public void checkMultipleSearch() throws Exception {
		...
		for (int i = 0; i < NUMBER_OF_ROOT_AGGREGATIONS; i++) {
			Node result = distributionService.findRootAggregationNode(
					parentNode, getDistribution(DISTRIBUTION_NAME + i));

			assertNotNull("Result should not be null", result);

			assertEquals("Incorrect type of node",
					DistributionNodeTypes.ROOT_AGGREGATION,
					NodeTypeManager.getType(DistributionService
							.getNodeType(result)));
			assertEquals("Incorrect count of node", NUMBER_OF_BARS,
					result.getProperty(DistributionService.COUNT));
			assertEquals("Incorrect name of node", DISTRIBUTION_NAME + i,
					result.getProperty(DistributionService.NAME));
		}
	}
	...
	public void checkNoExceptionsOnCreate() throws Exception {
		distributionService.createRootAggregationNode(getParentNode(),
				distribution);
	}

	@Test
	public void checkPropertiesOfCreatedRootAggregationNode() throws Exception {
		Node result = distributionService.createRootAggregationNode(
				getParentNode(), distribution);

		assertNotNull("Result should not be null", result);

		assertEquals("Incorrect name of Root Aggregation Name",
				DISTRIBUTION_NAME, result.getProperty(DistributionService.NAME));
		assertEquals("Incorrect count of Root Aggregation Name", 0,
				result.getProperty(DistributionService.COUNT));
		assertEquals("Incorrect type of Root Aggregation Name",
				DistributionNodeTypes.ROOT_AGGREGATION.getId(),
				result.getProperty(DistributionService.TYPE));
	}

	@Test
	public void checkRelationshipOfCreatedRootAggregationNode()
			throws Exception {
		Node result = distributionService.createRootAggregationNode(
				getParentNode(), distribution);

		Iterator<Relationship> relationships = result.getRelationships()
				.iterator();
	...
	public void checkRootAggregationRelationshipProperties() throws Exception {
		Node parentNode = getParentNode();
		Node result = distributionService.createRootAggregationNode(parentNode,
				distribution);
		...
		assertEquals("Invalid type of reationship",
				DistributionRelationshipTypes.ROOT_AGGREGATION.name(),
				relationship.getType().name());
		assertEquals("Invalid Direction of relationship", result,
				relationship.getEndNode());
		assertEquals("Invalid start node of relationship", parentNode,
	...
	public void chechNumberOfAggregationBarsInEmptyDB() throws Exception {
		Node rootNode = createRootAggregationNode(getParentNode(),
				DISTRIBUTION_NAME);

		Iterable<Node> aggregationBars = distributionService
				.findAggregationBars(rootNode);

		assertNotNull("Result should not be empy", aggregationBars);

		assertFalse("Result should not contain any nodes", aggregationBars
				.iterator().hasNext());
	}
	...
	public void checkNumberOfExistingBars() throws Exception {
		...
		Iterator<Node> aggregationBars = distributionService
				.findAggregationBars(rootNode).iterator();
	...
	public void checkPropertiesOfBars() throws Exception {
		...
		for (Node bar : aggregationBars) {
			...
			assertEquals("Unexpected type of Bar",
					DistributionNodeTypes.AGGREGATION_BAR,
					NodeTypeManager.getType(DistributionService
							.getNodeType(bar)));
			if (i != 0) {
				assertTrue("Unexpected color of Bar", Arrays.equals(
						getColorArray(new Color(i)),
						(int[]) bar.getProperty(DistributionService.BAR_COLOR)));
			} else {
				assertFalse("Unexpected color property",
						bar.hasProperty(DistributionService.BAR_COLOR));
			}
			assertEquals("Unexpected name of Bar", DISTRIBUTION_NAME + i,
					bar.getProperty(DistributionService.NAME));
			assertEquals("Unexpected count of Bar", i,
					bar.getProperty(DistributionService.COUNT));
	...
	public void tryToCreateAggregationBarWithoutRootNode() throws Exception {
		distributionService.createAggregationBarNode(null,
				getDistributionBar(DEFAULT_BAR_NAME));
	}
	...
	public void tryToCreateAggregationBarWithoutName() throws Exception {
		...
		distributionService.createAggregationBarNode(rootNode,
				getDistributionBar(null));
	}
	...
	public void tryToCreateAggregationBarWithEmptyName() throws Exception {
		...
		distributionService.createAggregationBarNode(rootNode,
				getDistributionBar(StringUtils.EMPTY));
	}
	...
	public void tryToCreateAggregationBarWithNegativeCount() throws Exception {
		...
		distributionService.createAggregationBarNode(rootNode,
				getDistributionBar(DISTRIBUTION_NAME, -DEFAULT_BAR_COUNT));
	}
	...
	public void checkPropertiesOfCreatedBar() throws Exception {
		...
		Node barNode = distributionService.createAggregationBarNode(rootNode,
				getDistributionBar(DEFAULT_BAR_NAME));

		assertNotNull("Result of creation should not be null", barNode);

		assertEquals("Invalid type of Bar Node",
				DistributionNodeTypes.AGGREGATION_BAR,
				NodeTypeManager.getType(DistributionService
						.getNodeType(barNode)));
		assertEquals("Invalid name of Bar Node", DEFAULT_BAR_NAME,
				barNode.getProperty(DistributionService.NAME));
		assertEquals("Invalid count of Bar Node", DEFAULT_BAR_COUNT,
				barNode.getProperty(DistributionService.COUNT));
	}
	...
	public void checkRelationshipCountOfSingleBar() throws Exception {
		...
		Iterator<Relationship> relationships = barNode.getRelationships()
				.iterator();
	...
	public void checkRelationshipTypeOfSingleBar() throws Exception {
		...
		assertEquals("Invalid type of reationship",
				DatasetRelationTypes.CHILD.name(), relationship.getType()
						.name());
		assertEquals("Invalid Direction of relationship", barNode,
				relationship.getEndNode());
		assertEquals("Invalid start node of relationship", rootNode,
				relationship.getStartNode());
	}
	...
	public void checkRelationshipsForMultiBar() throws Exception {
		...
		for (int i = 0; i < NUMBER_OF_BARS; i++) {
			barNodes[i] = distributionService.createAggregationBarNode(
					rootNode, getDistributionBar(DEFAULT_BAR_NAME + i));
		}
		...
		for (int i = 0; i < NUMBER_OF_BARS; i++) {
			...
			if (i == 0) {
				// first element - need to check CHILD incoming and NEXT
			...
			// check incoming
			Relationship incomingLink = barNode.getSingleRelationship(incoming,
					Direction.INCOMING);
			assertNotNull("No incoming <" + incoming + "> relationship",
					incoming);
			// check previous node
			...
			// check outgoing
			if (outgoing != null) {
				Relationship outgoingLink = barNode.getSingleRelationship(
						outgoing, Direction.OUTGOING);
				assertNotNull("No outgoing <" + outgoing + "> relationship",
						outgoingLink);
				// check next node
	...
	public void checkRelationshipBetweenBarAndSource() throws Exception {
		...
		assertTrue("BarNode should have at least one outgouing relationship",
				barNode.hasRelationship(
						DistributionRelationshipTypes.AGGREGATED,
						Direction.OUTGOING));
	}
	...
	public void checkNodeOfAggregationRelationship() throws Exception {
		...
		Relationship relFromBar = barNode.getSingleRelationship(
				DistributionRelationshipTypes.AGGREGATED, Direction.OUTGOING);
		Relationship relToSource = sourceNode.getSingleRelationship(
				DistributionRelationshipTypes.AGGREGATED, Direction.INCOMING);

		assertTrue(
				"Incorrect incoming and outgoing relationship for Aggregation",
				relFromBar.equals(relToSource));
	}
	...
	public void tryToUpdateDistirubitonBarWithoutNode() throws Exception {
		distributionService.updateDistributionBar(null,
				getDistributionBarInstance(null, UPDATED_BAR_NAME, true));
	}
	...
	public void tryToUpdateBarFromIncorrectDataStructure() throws Exception {
		...
		rootNode = createRootAggregationNode(getParentNode(), DISTRIBUTION_NAME
				+ UPDATED_BAR_NAME);

		IDistributionBar bar = getDistributionBarInstance(barNodes.get(1),
				UPDATED_BAR_NAME, true);
	...
	public void tryToSetEmptyNameForBar() throws Exception {
		...
		IDistributionBar bar = getDistributionBarInstance(barNode,
	...
	public void tryToUpdatePropertyWithoutRootElementNode() throws Exception {
		...
		IDistributionBar bar = getDistributionBarInstance(null,
				UPDATED_BAR_NAME, true);
	...
	public void tryToSetNegativeCountForBar() throws Exception {
		...
		IDistributionBar bar = getDistributionBarInstance(barNode,
				UPDATED_BAR_NAME, true, -UPDATED_BAR_COUNT);
	...
	public void checkUpdatedProperties() throws Exception {
		...
		assertEquals("incorrect name of bar", UPDATED_BAR_NAME,
				barNode.getProperty(AbstractService.NAME));
		assertEquals("incorrect name of bar", UPDATED_BAR_COUNT,
				barNode.getProperty(DistributionService.COUNT));
		assertTrue("incorrect color of bar", Arrays.equals(
				getColorArray(UPDATED_BAR_COLOR),
				(int[]) barNode.getProperty(DistributionService.BAR_COLOR)));
	}

	@Test(expected = IllegalArgumentException.class)
	public void tryToUpdateModelCountWithoutRootNode() throws Exception {
		distributionService.updateDistributionModelCount(null,
				UPDATED_BAR_COUNT);
	}
	...
	public void checkUpdatedCount() throws Exception {
		...
		distributionService.updateDistributionModelCount(rootNode,
				UPDATED_BAR_COUNT);

		assertEquals("Incorrect updated count", UPDATED_BAR_COUNT,
				rootNode.getProperty(DistributionService.COUNT));
	}
	...
	@Test(expected = IllegalArgumentException.class)
	public void tryToSetModelAsCurrentWithoutAnalyzedModelRoo()
			throws Exception {
		Node rootNode = createRootAggregationNode(getParentNode(),
				DISTRIBUTION_NAME);
	...
	public void setModelAsCurrent() throws Exception {
		...
		assertEquals("Unexpected root of current distribution",
				distributionService.getCurrentDistribution(parentNode),
				rootNode);
	}
	...
	public void skipCurrentModel() throws Exception {
		...
		assertFalse(
				"Parent should not have such property",
				parentNode
						.hasProperty(DistributionService.CURRENT_DISTRIBUTION_MODEL));
	}
	...
	public void tryToSetColorsWhenRootNodeIsNull() throws Exception {
		distributionService.updateSelectedBarColors(null, DEFAULT_LEFT_COLOR,
				DEFAULT_RIGHT_COLOR, DEFAULT_SELECTED_COLOR);
	}
	...
	public void tryToSetColorsWhenLeftColorIsNull() throws Exception {
		...
		distributionService.updateSelectedBarColors(rootNode, null,
				DEFAULT_RIGHT_COLOR, DEFAULT_SELECTED_COLOR);
	}
	...
	public void tryToSetColorsWhenRightColorIsNull() throws Exception {
		...
		distributionService.updateSelectedBarColors(rootNode,
				DEFAULT_LEFT_COLOR, null, DEFAULT_SELECTED_COLOR);
	}
	...
	public void tryToSetColorsWhenSelectedColorIsNull() throws Exception {
		...
		distributionService.updateSelectedBarColors(rootNode,
				DEFAULT_LEFT_COLOR, DEFAULT_RIGHT_COLOR, null);
	}
	...
	public void checkColorPropertiesOfNode() throws Exception {
		...
		distributionService
				.updateSelectedBarColors(rootNode, DEFAULT_LEFT_COLOR,
						DEFAULT_RIGHT_COLOR, DEFAULT_SELECTED_COLOR);

		assertTrue("Unexpected Left Color", Arrays.equals(
				getColorArray(DEFAULT_LEFT_COLOR),
				(int[]) rootNode.getProperty(DistributionService.LEFT_COLOR)));
		assertTrue("Unexpected Right Color", Arrays.equals(
				getColorArray(DEFAULT_RIGHT_COLOR),
				(int[]) rootNode.getProperty(DistributionService.RIGHT_COLOR)));
		assertTrue("Unexpected Selected Color", Arrays.equals(
				getColorArray(DEFAULT_SELECTED_COLOR), (int[]) rootNode
						.getProperty(DistributionService.SELECTED_COLOR)));
	}
	...
	public void checkPaletteChanges() throws Exception {
		...
		assertEquals("Unexpected name of Palette", DEFAULT_PALETTE.getName(),
				rootNode.getProperty(DistributionService.PALETTE));
	}
	...
	public void checkCreateUserDefinedDistributionRelations() throws Exception {
		...
		Node res = distributionService
				.createUserDefinedDistribution(distribution);
		assertNotNull("Relationship DISTRIBUTION not found",
				res.getRelationships(UserDefinedDistrRelTypes.DISTRIBUTION,
						Direction.INCOMING));
		assertNotNull("Relationship DATA not found", res.getSingleRelationship(
				UserDefinedDistrRelTypes.DATA, Direction.OUTGOING));
		Relationship barsRel = res.getSingleRelationship(
				UserDefinedDistrRelTypes.BARS, Direction.OUTGOING);
		assertNotNull("Relationship BARS not found", barsRel);
		Relationship barRel = barsRel.getEndNode().getSingleRelationship(
				UserDefinedDistrRelTypes.BAR, Direction.OUTGOING);
		assertNotNull("Relationship BAR not found", barRel);
		if (distribution.getBars().getBar().get(0).getColor() != null) {
			assertNotNull(
					"Relationship COLOR not found",
					barRel.getEndNode().getSingleRelationship(
							UserDefinedDistrRelTypes.COLOR, Direction.OUTGOING));
		} else {
			assertNull(
					"Relationship COLOR found",
					barRel.getEndNode().getSingleRelationship(
							UserDefinedDistrRelTypes.COLOR, Direction.OUTGOING));
		}
		Relationship filterRel = barRel.getEndNode().getSingleRelationship(
				UserDefinedDistrRelTypes.FILTER, Direction.OUTGOING);
		assertNotNull("Relationship FILTER from BAR Node not found", filterRel);
		if (distribution.getBars().getBar().get(0).getFilter()
				.getUnderlyingFilter() != null) {
			assertNotNull(
					"Relationship FILTER from FILTER Node not found",
					filterRel.getEndNode()
							.getSingleRelationship(
									UserDefinedDistrRelTypes.FILTER,
									Direction.OUTGOING));
		} else {
			assertNull(
					"Relationship FILTER from FILTER Node found",
					filterRel.getEndNode()
							.getSingleRelationship(
									UserDefinedDistrRelTypes.FILTER,
									Direction.OUTGOING));
	...
	public void checkCreateUserDefinedDistributionProperties() throws Exception {
		...
		Node res = distributionService
				.createUserDefinedDistribution(distribution);
		Node refNode = res.getSingleRelationship(
				UserDefinedDistrRelTypes.DISTRIBUTION, Direction.INCOMING)
				.getStartNode();
		assertEquals("Reference node not found", refNode,
				distributionService.getReferenceNode());
		Node data = res.getSingleRelationship(UserDefinedDistrRelTypes.DATA,
				Direction.OUTGOING).getEndNode();

		checkProperty(DistributionService.UD_NAME, data, distribution.getData()
				.getName(), true);
		checkProperty(DistributionService.UD_DATA_TYPE, data, distribution
				.getData().getDataType(), true);
		checkProperty(DistributionService.UD_NODE_TYPE, data, distribution
				.getData().getNodeType(), true);
		checkProperty(DistributionService.UD_PROPERTY_NAME, data, distribution
				.getData().getPropertyName(), false);
		checkProperty(DistributionService.UD_DRIVE_TYPE, data, distribution
				.getData().getDriveType(), false);

		Node barNode = res
				.getSingleRelationship(UserDefinedDistrRelTypes.BARS,
						Direction.OUTGOING)
				.getEndNode()
				.getSingleRelationship(UserDefinedDistrRelTypes.BAR,
						Direction.OUTGOING).getEndNode();
		Bar bar = distribution.getBars().getBar().get(0);

		checkProperty(DistributionService.UD_BAR_NAME, barNode, bar.getName(),
				true);
		if (bar.getColor() != null) {
			Node colorNode = barNode.getSingleRelationship(
					UserDefinedDistrRelTypes.COLOR, Direction.OUTGOING)
					.getEndNode();
			checkProperty(DistributionService.UD_BAR_COLOR_RED, colorNode, bar
					.getColor().getRed(), true);
			checkProperty(DistributionService.UD_BAR_COLOR_GREEN, colorNode,
					bar.getColor().getGreen(), true);
			checkProperty(DistributionService.UD_BAR_COLOR_BLUE, colorNode, bar
					.getColor().getBlue(), true);
		}

		Node filterNode = barNode.getSingleRelationship(
				UserDefinedDistrRelTypes.FILTER, Direction.OUTGOING)
				.getEndNode();
	...
	private void checkUserDefinedDistributionFilter(Node filterNode,
			Filter filter) {
		checkProperty(DistributionService.UD_FILTER_TYPE, filterNode,
				filter.getFilterType(), false);
		checkProperty(DistributionService.UD_FILTER_EXP_TYPE, filterNode,
				filter.getExpressionType(), false);
		checkProperty(DistributionService.UD_FILTER_NODE_TYPE, filterNode,
				filter.getNodeType(), true);
		checkProperty(DistributionService.UD_PROPERTY_NAME, filterNode,
				filter.getPropertyName(), true);
		checkProperty(DistributionService.UD_FILTER_VALUE, filterNode,
				filter.getValue(), false);
		if (filter.getUnderlyingFilter() != null) {
			Node underFilterNode = filterNode.getSingleRelationship(
					UserDefinedDistrRelTypes.FILTER, Direction.OUTGOING)
					.getEndNode();
			checkUserDefinedDistributionFilter(underFilterNode,
					filter.getUnderlyingFilter());
		}
	}
	...
	public void checkUserDefinedDistributionsSize() throws Exception {
		...
		List<Distribution> distributions = distributionService
				.findUserDefinedDistributions();

		assertEquals("Incorrect User Defined Distributions count",
				USER_DEFINED_DISTR_NAMES.length, distributions.size());
	}
	...
	public void checkUserDefinedDistributions() throws Exception {
		...
		assertNotNull("Data of Distribution should be not null",
				distrAfter.getData());
		checkProperty(DistributionService.UD_NAME, dataBefore.getName(),
				dataAfter.getName(), true);
		checkProperty(DistributionService.UD_DATA_TYPE,
				dataBefore.getDataType(), dataAfter.getDataType(), true);
		checkProperty(DistributionService.UD_NODE_TYPE,
				dataBefore.getNodeType(), dataAfter.getNodeType(), true);
		checkProperty(DistributionService.UD_PROPERTY_NAME,
				dataBefore.getPropertyName(), dataAfter.getPropertyName(),
				false);
		checkProperty(DistributionService.UD_DRIVE_TYPE,
				dataBefore.getDriveType(), dataAfter.getDriveType(), false);

		assertNotNull("Bars of Distribution should be not null",
				distrAfter.getBars());
		assertEquals("Incorrect Bar of Distribution count", distrBefore
				.getBars().getBar().size(), distrAfter.getBars().getBar()
				.size());

		Bar barBefore = distrBefore.getBars().getBar().get(0);
		Bar barAfter = distrAfter.getBars().getBar().get(0);
		checkProperty(DistributionService.UD_BAR_NAME, barBefore.getName(),
				barAfter.getName(), true);
		if (barBefore.getColor() != null) {
			assertNotNull("Bar Color should be not null", barAfter.getColor());
			checkProperty(DistributionService.UD_BAR_COLOR_RED, barBefore
					.getColor().getRed(), barAfter.getColor().getRed(), true);
			checkProperty(DistributionService.UD_BAR_COLOR_GREEN, barBefore
					.getColor().getGreen(), barAfter.getColor().getGreen(),
					true);
			checkProperty(DistributionService.UD_BAR_COLOR_BLUE, barBefore
					.getColor().getBlue(), barAfter.getColor().getBlue(), true);
		}

		assertNotNull("Filter should be not null", barAfter.getFilter());
		checkUserDefinedDistributionFilter(barBefore.getFilter(),
				barAfter.getFilter());
	}

	@Test
	public void checkFindBarByAggregatedNode() {
		Node aggregatedNode = getNodeMock();
		long parentId = 0l;
		Relationship relationship = mock(Relationship.class);
		Node node = mock(Node.class);
		List<Relationship> relationships = Arrays.asList(relationship);
		when(
				aggregatedNode.getRelationships(Direction.INCOMING,
						DistributionRelationshipTypes.AGGREGATED)).thenReturn(
				relationships);

		when(relationship.getOtherNode(aggregatedNode)).thenReturn(node);
		when(node.getProperty(DatasetService.PARENT_ID, 0l)).thenReturn(
				parentId);

		assertEquals(node, distributionService.findBarByAggregatedNode(
				aggregatedNode, parentId));

		when(node.getProperty(DatasetService.PARENT_ID, 0l)).thenReturn(2l);

		assertNull(distributionService.findBarByAggregatedNode(aggregatedNode,
				parentId));
	}

	@Test(expected = IllegalArgumentException.class)
	public void checkFindBarByAggregatedNodeWithNullArg() {
		distributionService.findBarByAggregatedNode(null, 0l);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	@Test
	public void checkFindBarByAggregatedNodeWithNullRelationship() {
		Node aggregatedNode = getNodeMock();
		Relationship relationship = mock(Relationship.class);
		List relationships = mock(List.class);
		relationships.add(relationship);
		when(
				aggregatedNode.getRelationships(Direction.INCOMING,
						DistributionRelationshipTypes.AGGREGATED)).thenReturn(
				relationships);
		when(relationships.iterator()).thenReturn(null);
		assertNull(distributionService.findBarByAggregatedNode(aggregatedNode,
				0l));
	}

	@Test(expected = IllegalArgumentException.class)
	public void checkUpdateDistributionBar() throws DatabaseException {
		IDistributionBar distributionBar = mock(IDistributionBar.class);
		when(distributionBar.getName()).thenReturn("name");
		when(distributionBar.getCount()).thenReturn(3);
		when(distributionBar.getRootElement()).thenReturn(null);
		Node rootAggregationNode = getNodeMock();
		distributionService.updateDistributionBar(rootAggregationNode,
				distributionBar);
	}

	@Test
	public void checkGetCurrentDistribution() throws DatabaseException {
		Node node = getNodeMock();
		when(
				node.getProperty(
						DistributionService.CURRENT_DISTRIBUTION_MODEL, null))
				.thenReturn(null);
		assertNull(distributionService.getCurrentDistribution(node));
		...
		when(
				node.getRelationships(
						DistributionRelationshipTypes.ROOT_AGGREGATION,
						Direction.OUTGOING)).thenReturn(
				new ArrayList<Relationship>());
		distributionService.getCurrentDistribution(node);
	}

	@Test
	public void checkCreateUserDefinedDistribution() throws DatabaseException {
		Distribution distribution = createXmlDistribution();
		Data data = distribution.getData();
		data.setDriveType(DriveTypes.NEMO_V2.toString());
		distribution.setData(data);

		distributionService
				.createUserDefinedDistribution((Distribution) distribution);
	}

	@Test(expected = IllegalArgumentException.class)
	public void checkGetCurrentDistributionWithNull() throws DatabaseException {
		distributionService.getCurrentDistribution(null);
	}
	
	private void checkUserDefinedDistributionFilter(Filter filterBefore,
			Filter filterAfter) {
		checkProperty(DistributionService.UD_FILTER_TYPE,
				filterBefore.getFilterType(), filterAfter.getFilterType(),
				false);
		checkProperty(DistributionService.UD_FILTER_EXP_TYPE,
				filterBefore.getExpressionType(),
				filterAfter.getExpressionType(), false);
		checkProperty(DistributionService.UD_FILTER_NODE_TYPE,
				filterBefore.getNodeType(), filterAfter.getNodeType(), true);
		checkProperty(DistributionService.UD_PROPERTY_NAME,
				filterBefore.getPropertyName(), filterAfter.getPropertyName(),
				true);
		checkProperty(DistributionService.UD_FILTER_VALUE,
				filterBefore.getValue(), filterAfter.getValue(), false);
		if (filterBefore.getUnderlyingFilter() != null) {
			assertNotNull("Underlying Filter should be not null",
					filterAfter.getUnderlyingFilter());
			checkUserDefinedDistributionFilter(
					filterBefore.getUnderlyingFilter(),
					filterAfter.getUnderlyingFilter());
		}
	}

	private void checkProperty(String propertyName, Object valueBefore,
			Object valueAfter, boolean mandatory) {
	...
	public void checkProperty(String propertyName, Node node, Object value,
			boolean mandatory) {
		if (mandatory || value != null) {
			assertNotNull(propertyName + " should be not null",
					node.getProperty(propertyName, null));
			assertEquals("Incorrect " + propertyName,
					node.getProperty(propertyName), value);
		} else {
			assertNull(propertyName + " should be null",
					node.getProperty(propertyName, null));
		}
	}

	private IDistributionBar getDistributionBarInstance(Node barNode,
			String name, boolean createRootElement, int count) {
		DistributionBar result = getDistributionBarInstance(barNode, name,
				createRootElement);
	 ...
	 * @return
	 */
	private DistributionBar getDistributionBarInstance(Node barNode,
			String name, boolean createRootElement) {
		DistributionBar result = new DistributionBar(
				mock(IDistributionModel.class));
	...
	private List<Node> createAggregationBars(Node rootNode) throws Exception {
		...
		try {
			...
			for (int i = 0; i < NUMBER_OF_BARS; i++) {
				...
				bar.setProperty(DistributionService.TYPE,
						DistributionNodeTypes.AGGREGATION_BAR.getId());
				bar.setProperty(DistributionService.COUNT, i);
				bar.setProperty(DistributionService.NAME, DISTRIBUTION_NAME + i);

				if (previousBar == null) {
					rootNode.createRelationshipTo(bar,
							DatasetRelationTypes.CHILD);
				} else {
					previousBar.createRelationshipTo(bar,
							DatasetRelationTypes.NEXT);
				}

				if (i != 0) {
					bar.setProperty(DistributionService.BAR_COLOR,
							getColorArray(new Color(i)));
				}
	 ...
	 * @throws Exception
	 */
	private Node createRootAggregationNode(Node parentNode, String name)
			throws Exception {
		Transaction tx = graphDatabaseService.beginTx();
		...
		try {
			...
			parentNode.createRelationshipTo(result,
					DistributionRelationshipTypes.ROOT_AGGREGATION);

			result.setProperty(AbstractService.TYPE,
					DistributionNodeTypes.ROOT_AGGREGATION.getId());
			result.setProperty(AbstractService.NAME, name);
			result.setProperty(DistributionService.NODE_TYPE,
					DEFAULT_NODE_TYPE.getId());
			result.setProperty(DistributionService.PROPERTY_NAME,
	...
	protected Distribution createXmlDistribution(String name) {
		...
		bar.setColor(new org.amanzi.neo.model.distribution.xml.schema.Color(
				DEF_COLOR_RED, DEF_COLOR_GREEN, DEF_COLOR_BLUE));
