...
public class ProjectModelTest extends AbstractNeoServiceTest {
    ...
    private List<Node> getNetworkNodes(Integer count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            when(networkNode.getProperty(NewNetworkService.TYPE, null)).thenReturn(DatasetTypes.NETWORK.getId());
            when(networkNode.getProperty(NewNetworkService.NAME, StringUtils.EMPTY)).thenReturn(count.toString());
            when(networkNode.getRelationships(PropertyIndex.NeoIndexRelationshipTypes.INDEX, Direction.OUTGOING)).thenReturn(
