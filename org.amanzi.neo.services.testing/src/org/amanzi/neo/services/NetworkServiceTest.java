...
import org.amanzi.neo.services.AbstractService.FilterRelationshipType;
...
public class NetworkServiceTest extends AbstractAWETest {
    ...
    public void testGetIndexKey() {
        ...
        for (INodeType type : NetworkElementNodeType.values()) {
            String key = AbstractService.getIndexKey(parent, type);
    ...
    public void testGetIndexKeyRootNull() {
        // exception
        AbstractService.getIndexKey(null, NetworkElementNodeType.values()[0]);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testGetIndexKeyTypeNull() {
        // exception
        AbstractService.getIndexKey(parent, null);
    }
        ...
        for (INodeType type : NetworkElementNodeType.values()) {
            ...
            try {
                ...
                Assert.assertEquals(parent, node.getRelationships(DatasetRelationTypes.CHILD, Direction.INCOMING).iterator().next()
                        .getOtherNode(node));
                // all properties set
                Assert.assertEquals(type.getId(), node.getProperty(NetworkService.NAME, null));
                Assert.assertEquals(type.getId(), node.getProperty(NetworkService.TYPE, null));
                // the element is indexed
                Assert.assertEquals(node, index.get(NetworkService.NAME, type.getId()).getSingle());
    ...
    public void testFindNetworkElement() {
        ...
        for (INodeType type : NetworkElementNodeType.values()) {
            ...
            Assert.assertEquals(type.getId(), node.getProperty(AbstractService.NAME, null));
            // the relationship from parent exists
            Assert.assertEquals(parent, node.getRelationships(DatasetRelationTypes.CHILD, Direction.INCOMING).iterator().next()
                    .getOtherNode(node));
            // node is correct
    ...
    public void testGetNetworkElement() {
        ...
        try {
            ...
            // name is right
            Assert.assertEquals(type.getId(), testNode.getProperty(NetworkService.NAME, null));
    ...
    public void testCreateSector() {
        ...
        try {
            parent = getNewNE();
            parent.setProperty(NetworkService.TYPE, NetworkElementNodeType.SITE.getId());
        ...
        Assert.assertEquals("name", sector.getProperty(NetworkService.NAME, null));
        Assert.assertEquals("ci", sector.getProperty(NetworkService.CELL_INDEX, null));
        Assert.assertEquals("lac", sector.getProperty(NetworkService.LOCATION_AREA_CODE, null));
        Assert.assertEquals(NetworkElementNodeType.SECTOR.getId(), sector.getProperty(NetworkService.TYPE, null));
        ...
        Assert.assertEquals(parent, sector.getRelationships(DatasetRelationTypes.CHILD, Direction.INCOMING).iterator().next()
                .getOtherNode(sector));
        // sector is indexed
        index = graphDatabaseService.index().forNodes(AbstractService.getIndexKey(parent, NetworkElementNodeType.SECTOR));
        Assert.assertEquals(sector, index.get(NetworkService.NAME, "name").getSingle());
        Assert.assertEquals(sector, index.get(NetworkService.LOCATION_AREA_CODE, "lac").getSingle());
        Assert.assertEquals(sector, index.get(NetworkService.CELL_INDEX, "ci").getSingle());
    }
    ...
    public void testGetSectorNoSector() {
        ...
        try {
            ...
            Assert.assertNotNull(testNode);
            // all params correct
            Assert.assertEquals("name", testNode.getProperty(NetworkService.NAME, null));
            Assert.assertEquals("ci", testNode.getProperty(NetworkService.CELL_INDEX, null));
            Assert.assertEquals("lac", testNode.getProperty(NetworkService.LOCATION_AREA_CODE, null));
            Assert.assertEquals(NetworkElementNodeType.SECTOR.getId(), testNode.getProperty(NetworkService.TYPE, null));
            // relation from parent is created
            Assert.assertEquals(parent, testNode.getRelationships(DatasetRelationTypes.CHILD, Direction.INCOMING).iterator().next()
                    .getOtherNode(testNode));
            // sector is indexed
            Assert.assertEquals(testNode, index.get(NetworkService.NAME, "name").getSingle());
            Assert.assertEquals(testNode, index.get(NetworkService.LOCATION_AREA_CODE, "lac").getSingle());
            Assert.assertEquals(testNode, index.get(NetworkService.CELL_INDEX, "ci").getSingle());
        } catch (AWEException e) {
            LOGGER.error("could not create sector", e);
            fail();
        }
    ...
    public void checkNameOfCreatedSelectionListNode() {
        ...
        Assert.assertEquals(DEFAULT_SELECTION_LIST_NAME, selectionNode.getProperty(AbstractService.NAME));
    }
    ...
    public void checkTypeOfCreatedLink() throws Exception {
        ...
        assertEquals("Incorrect type of Relationship", NetworkRelationshipTypes.SELECTED.name(), linkType.name());
    }
    ...
    public void checkReplaceRelationship() throws Exception {
        ...
        networkService.createRelationship(root, childNode, DatasetRelationTypes.CHILD);
        Node newRootNode = getNewNE();
        networkService.replaceRelationship(newRootNode, childNode, DatasetRelationTypes.CHILD, Direction.INCOMING);
        assertNull(root + " still has relationships", root.getSingleRelationship(DatasetRelationTypes.CHILD, Direction.OUTGOING));
        assertNotNull(newRootNode + "still hasn't relationships",
                newRootNode.getSingleRelationship(DatasetRelationTypes.CHILD, Direction.OUTGOING));
    }
    ...
    public void checkSavedNetworkStructureInNode() throws Exception {
        ...
        assertTrue("No network structure in node", network.hasProperty(NetworkService.NETWORK_STRUCTURE));
    }
    ...
    public void checkContentOfNetworkStructureInNode() throws Exception {
        ...
        String[] structureFromNode = (String[])network.getProperty(NetworkService.NETWORK_STRUCTURE);
    ...
    public void checkCreateProxyRelation() throws DatabaseException {
        ...
        props.put(AbstractService.NAME, TEST_NAME);
    ...
    public void checkCreateProxyProperty() throws DatabaseException {
        ...
        assertTrue("Proxy node doesn't have " + NetworkService.SOURCE_NAME + " property",
                result.hasProperty(NetworkService.SOURCE_NAME));

        assertTrue(NetworkService.SOURCE_NAME + " property of proxy node contains incorrect data",
                TEST_NAME.equals(result.getProperty(NetworkService.SOURCE_NAME)));
    ...
    public void checkFindSelectionLink() throws AWEException {
        ...
        assertEquals(NetworkRelationshipTypes.SELECTED.name(),
                networkService.findSelectionLink(selectionRootNode, selectedNode, linkIndex).getType().name());
    }
    ...
    @Test
    public void testSaveSingleFilterWithValueNode() {
        try {
            ...
            LOGGER.error("testSaveSingleFilterWithValueNode() finished with exception ", e);
    ...
    @Test
    public void testSaveSingleFilterWithoutValueNode() {
        try {
            Node parentNode = networkService.createNode(NetworkElementNodeType.SECTOR);
            networkService.setAnyProperty(parentNode, NetworkService.NAME, NAME_VALUE);
            ...
            LOGGER.error("testSaveSingleFilterWithoutValueNode() finished with exception ", e);
            Assert.fail("Test finished with exception" + e);
        }
    ...
    @Test
    public void testSavingFilterChain() {
        try {
            Node parentNode = networkService.createNode(NetworkElementNodeType.SECTOR);
            networkService.setAnyProperty(parentNode, NetworkService.NAME, NAME_VALUE);
            IFilter filter = new Filter(FILTER_NAME);
            ...
            Assert.assertTrue("FILTER relationship expected ", parentNode.hasRelationship(FilterRelationshipType.FILTER));
            Assert.assertTrue("OR relationship expected ",
                    parentNode.getSingleRelationship(FilterRelationshipType.FILTER, Direction.OUTGOING).getEndNode()
                            .hasRelationship(FilterRelationshipType.OR));
        } catch (Exception e) {
            LOGGER.error("testSavingFilterChain() finished with exception ", e);
            Assert.fail("Test finished with exception" + e);
        }
    ...
    @Test
    public void testLoadingSingleFilter() {
        try {
            Node parentNode = networkService.createNode(NetworkElementNodeType.SECTOR);
            networkService.setAnyProperty(parentNode, NetworkService.NAME, NAME_VALUE);
            IFilter filter = new Filter(FILTER_NAME);
    ...
    @Test
    public void testLoadingFilterChain() {
        try {
            Node parentNode = networkService.createNode(NetworkElementNodeType.SECTOR);
            networkService.setAnyProperty(parentNode, NetworkService.NAME, NAME_VALUE);
            IFilter filter = new Filter(FILTER_NAME);
