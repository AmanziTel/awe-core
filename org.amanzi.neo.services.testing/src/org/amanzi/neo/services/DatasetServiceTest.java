 ...
 * @author kruglik_a
 */
public class DatasetServiceTest extends AbstractNeoServiceTest {

    private static Logger LOGGER = Logger.getLogger(DatasetServiceTest.class);
    ...
    public void createDatasetWithDriveTypeTest() throws InvalidDatasetParameterException, DatasetTypeParameterException,
        ...
        String actualType = (String)actualDataset.getProperty(DatasetService.TYPE);
        Assert.assertEquals("dataset has wrong type", DatasetTypes.DRIVE.getId(), actualType);

        String actualDriveType = (String)actualDataset.getProperty(DatasetService.DRIVE_TYPE);
    ...
    public void testAddChildNoChildren() {
        ...
        Assert.assertEquals(child.getId(), parent.getProperty(DatasetService.LAST_CHILD_ID, 0L));
        // parent_id set
        Assert.assertEquals(parent.getId(), child.getProperty(DatasetService.PARENT_ID, 0L));
        // valid chain exists
    ...
    public void testAddChildLastChild() {
        ...
        Transaction tx = graphDatabaseService.beginTx();
        try {
            parent.removeProperty(DatasetService.LAST_CHILD_ID);
            tx.success();
        } catch (Exception e) {
    ...
    public void testAddChildLCIdWrong() {
        ...
        Transaction tx = graphDatabaseService.beginTx();
        try {
            parent.setProperty(DatasetService.LAST_CHILD_ID, parent.getId());
            tx.success();
        } catch (Exception e) {
    ...
    public void testGetParentNoIdOneChild() {
        ...
        Transaction tx = graphDatabaseService.beginTx();
        try {
            child.removeProperty(DatasetService.PARENT_ID);
            tx.success();
        } catch (Exception e) {
            LOGGER.error("Could not set property", e);
            Assert.fail();
    ...
    public void testGetLastChildNoId() {
        ...
        Assert.assertTrue(chainExists(parent, lastChild));
        // last_child_id updated
        Assert.assertEquals(lastChild.getId(), parent.getProperty(DatasetService.LAST_CHILD_ID, 0L));
    }
    ...
    public void testAddChildNoChain() {
        ...
        for (Relationship rel : parent.getRelationships(DatasetRelationTypes.CHILD, Direction.OUTGOING)) {
    ...
    public void testGetElementByType() {
        ...
        try {
            for (Node[] nodds : nodes) {
                for (Node node : nodds) {
                    ...
                    node.setProperty(AbstractService.TYPE, type.getId());
                    switch (type) {
                    case FILE:
                        files.add(node);
                        break;
        ...
        for (DriveNodeTypes type : possibleDriveNodeTypes) {
            ...
            for (Node node : traverser) {
                DriveNodeTypes testType = DriveNodeTypes.findById(node.getProperty(AbstractService.TYPE, null).toString());
                // type correct
