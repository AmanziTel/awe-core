...
public class NeoGeoResourceInfo extends IGeoResourceInfo {
	...
	public NeoGeoResourceInfo(NeoGeoResource resource, Node gisNode,
		...
		try {
			this.name = gisNode.getProperty(INeoConstants.PROPERTY_NAME_NAME)
					.toString();
			this.description = gisNode
					.hasProperty(INeoConstants.PROPERTY_DESCRIPTION_NAME) ? gisNode
					.getProperty(INeoConstants.PROPERTY_DESCRIPTION_NAME)
					.toString()
					: "GeoNeo Data";
			this.bounds = geoNeo.getBounds();
		} catch (Exception e) {
			System.err.println("Failed to determine GeoResourceInfo: "
					+ e.getMessage());
			e.printStackTrace(System.err);
		}
		final ImageData imageData = IconManager.getIconManager().getImage(
				IconManager.NETWORK_ICON).getImageData();
