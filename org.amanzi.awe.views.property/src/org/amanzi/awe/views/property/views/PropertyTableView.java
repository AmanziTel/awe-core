...
public class PropertyTableView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		tableViewer = new TableViewer(frame, SWT.VIRTUAL | SWT.BORDER
				| SWT.FULL_SELECTION);
	...
	private void enableElements() {
		...
		cbFilter.setEnabled(cbData.getSelectionIndex() >= 0
				&& cbProperty.getSelectionIndex() >= 0);
	}
	...
	private class TableContentProvider implements IStructuredContentProvider {
		...
		public void inputChanged(Viewer viewer, Object oldInput, Object newInput) {
			...
			String[] properties = selModel.getAllPropertyNames(selModel
					.getType());
			Iterable<IDataElement> dataElements = selModel
					.getAllElementsByType(selModel.getType());
			...
			for (IDataElement element : dataElements) {
				...
				if (StringUtils.isNotEmpty(selProperty)
						&& StringUtils.isNotEmpty(selFilter)) {
					Class<?> propClass = selModel.getPropertyClass(
							selModel.getType(), selProperty);
					if (Number.class.isAssignableFrom(propClass)) {
						for (int i = 0; i < PropertyNumberFilter.values().length; i++) {
							if (selFilter
									.equals(PropertyNumberFilter.values()[i]
											.getCaption())) {
								if ((((DataElement) element).get(selProperty) == null)
										^ PropertyNumberFilter.values()[i]
												.isPropertyNull()) {
									doFilter = false;
									break;
								}
						...
						if (!selFilter.equals(((DataElement) element)
								.get(selProperty)))
							doFilter = false;
					}
				}
				if (doFilter) {
					...
					for (int q = 0; q < properties.length; q++) {
						row.addPropValue((((DataElement) element)
								.get(properties[q])));
					}
					elements.add(row);
				}
	...
	private void setProperties() {
		...
		if (dataModel == null) {
			...
			cbProperty.setItems(dataModel.getAllPropertyNames(dataModel
					.getType()));
		}
	}
	...
	private void setFilters() {
		...
		if (dataModel != null && !StringUtils.isEmpty(property)) {
			Class<?> propClass = dataModel.getPropertyClass(
					dataModel.getType(), property);
			if (Number.class.isAssignableFrom(propClass)) {
				...
				Object[] values = dataModel.getPropertyValues(
						dataModel.getType(), property);
