...
public class RakeOutlinePage extends RubyOutlinePage implements
	...
	protected IBaseLabelProvider getLabelProvider() {
		return new LabelProvider() {
			...
			public Image getImage(Object element) {
				ImageDescriptorRegistry registry = RubyPlugin
						.getImageDescriptorRegistry();
				ImageDescriptor descriptor = null;
				if (element instanceof Task) {
					descriptor = RakePlugin.imageDescriptorFromPlugin(
							RakePlugin.PLUGIN_ID, "icons/targetpublic_obj.gif");
				} else if (element instanceof Namespace) {
					descriptor = RakePlugin.imageDescriptorFromPlugin(
							RakePlugin.PLUGIN_ID, "icons/task_obj.gif");
				} else if (element instanceof IRubyElement) {
					descriptor = new RubyElementImageProvider()
							.getRubyImageDescriptor((IRubyElement) element, 0);
				}
	...
	class RSpecChildrenProvider extends ChildrenProvider {
		public Object[] getElements(Object inputElement) {
			...
			RootNode root = ASTProvider.getASTProvider().getAST(script,
			...
			try {
				...
				Object[] scriptChildren = filterOutLocalVars(script.getChildren());
				// TODO Filter out local variables!
				Object[] all = new Object[tasks.length + scriptChildren.length];

				System.arraycopy(scriptChildren, 0, all, 0,
						scriptChildren.length);
				System.arraycopy(tasks, 0, all, scriptChildren.length,
						tasks.length);
				return all;
		...
		private Object[] filterOutLocalVars(Object[] original) {
			List<Object> filtered = new ArrayList<Object>();
			for (int i = 0; i < original.length; i++) {
				if (original[i] instanceof ILocalVariable) {
					continue;
				}
		...
		public Object[] getChildren(Object parentElement) {
			...
			try {
				if (parentElement instanceof IParent)
					return filterOutLocalVars(((IParent) parentElement).getChildren());
			} catch (RubyModelException e) {
				RakePlugin.log(e);
			}
			return new Object[0];
