...
public class JRubyVMRunner extends StandardVMRunner implements IVMRunner {
	...
	protected Process exec(String[] cmdLine, File workingDirectory, String[] envp) throws CoreException {	
		...
		try {
		    ...
		    Status status = new Status(IStatus.ERROR, DebugPlugin.getUniqueIdentifier(), DebugPlugin.INTERNAL_ERROR, LaunchingMessages.DebugPlugin_Exception_occurred_executing_command_line__1, e); 
		    throw new CoreException(status);
		} catch (NoSuchMethodError e) {
			//attempting launches on 1.2.* - no ability to set working directory			
			IStatus status = new Status(IStatus.ERROR, DebugPlugin.getUniqueIdentifier(), DebugPlugin.ERR_WORKING_DIRECTORY_NOT_SUPPORTED, LaunchingMessages.DebugPlugin_Eclipse_runtime_does_not_support_working_directory_2, e); 
