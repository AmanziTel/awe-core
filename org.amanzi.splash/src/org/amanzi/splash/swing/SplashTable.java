...
public class SplashTable extends JTable {
    ...
    public void deleteRow(final int index) {
        ...
        final Boolean result = (Boolean)ActionUtil.getInstance().runTaskWithResult(new RunnableWithResult<Boolean>() {
            ...
            public void run() {
                ...
                if (!result) {
                    ErrorDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), ERROR_TITLE, ERROR_MSG, Status.CANCEL_STATUS, Status.CANCEL);
    ...
    public SelectedCellsSet copyCell() {
        ...
        StringSelection stsel = new StringSelection("qwe\tqweqweaa\nqwewqe");
        NeoSplashUtil.logn("Copy pressed !!!");
        CellSelection cs = new CellSelection(result);
        cl.setContents(stsel, stsel);
    ...
    public void pasteCell(int row, int column) {
        ...
        if (cl.isDataFlavorAvailable(CellSelection.CELL_DATA_FLAVOR)) {
            ...
            try {
                String stringData = (String)cl.getData(DataFlavor.stringFlavor);
                String[] rows = stringData.split("\n");
                int curRow = row;
                for (String stringRow : rows) {
                    int curCol = column;
                    String[] stringCells = stringRow.split("\t");
                    for (String stringCell : stringCells) {
                        // Cell cell = new Cell(curRow, curCol);
                        Cell cell = new Cell(curRow, curCol, stringCell, stringCell, new CellFormat());
                        // cell.setValue(stringCell);
                        getModel().setValueAt(cell, curRow, curCol);
                        curCol++;
                    }
                    curRow++;
                }
