...
public class RomesDriveModelSaver<T extends BaseTransferData> implements
	...
	private int count = 0;
	private GraphDatabaseService graphDb = NeoServiceProvider.getProvider()
			.getService();
	...
	public void init(T element) {
		...
		tx = graphDb.beginTx();
		try {
			Node project = NeoServiceFactory.getInstance()
					.getNewProjectService().getProject(projectName);
	...
	protected void definePropertyMap(BaseTransferData element) {
		...
		defineHeader(headers, "events", new String[] { "Event Type",
				"event_type" });
		defineHeader(headers, "time", new String[] { "time", "Timestamp",
				"timestamp" });
		defineHeader(headers, INeoConstants.SECTOR_ID_PROPERTIES,
				new String[] { ".*Server.*Report.*CI.*" });

	}

	protected void defineHeader(Set<String> headers, String newName,
		...
		for (String header : headers) {
			...
			for (String headerRegExp : possibleHeaders) {
				Pattern pat = Pattern.compile(headerRegExp,
