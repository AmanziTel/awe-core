...
public abstract class AbstractSaver<T1 extends IModel, T2 extends IData, T3 extends IConfiguration>
	...
	protected static Object autoParse(String propertyName, String propertyValue) {
		try {
			Object predifinedCheck = checkInPredifined(propertyName,
					propertyValue);
	...
	protected void createExportSynonymsForModels() throws AWEException {
		try {
			for (IModel model : useableModels) {
				synonymsMap.put(model, exportManager.createExportSynonym(model,
						ExportSynonymType.DATASET));
			}
		} catch (DatabaseException e) {
			AweConsolePlugin
					.error("Error while creating export synonyms for models");
	 ...
	 * @param synonym
	 */
	protected void addDatasetSynonyms(IDataModel model, INodeType nodeType,
	...
	public void finishUp() throws AWEException {
		List<IRenderableModel> renderableList = new ArrayList<IRenderableModel>(
				useableModels.size());
		for (IDataModel dataModel : useableModels) {
			dataModel.finishUp();
			if (dataModel instanceof IRenderableModel) {
				renderableList.add((IRenderableModel) dataModel);				
			}
		}
		saveSynonym();
		...
		if (isRenderable()) {
			EventManager.getInstance().fireEvent(
					new ShowOnMapEvent(renderableList, 900d));
		}
	}
