 ...
 * @author Bondoronok_P
 */
public class Nemo2xSaver extends
		AbstractMappedDataSaver<IDriveModel, NemoConfiguration> {
	...
	protected void saveElement(MappedData dataElement) throws AWEException {
		...
		Map<String, Object> values = getDataElementProperties(model, eventId,
				dataElement, true, false);
	...
	@Override
	protected IDriveModel createMainModel(NemoConfiguration configuration)
			throws AWEException {
		return getActiveProject().createDriveModel(
				configuration.getDatasetName(), DriveTypes.NEMO_V2);
	}
