...
public class TRXSaver extends AbstractNetworkSaver {
	...
	protected void saveLine(List<String> value) throws AWEException {
		if (frequency_rootElement == null) {
			...
			frequency.put(NewAbstractService.TYPE,
					NetworkElementNodeType.FREQUENCY_ROOT.getId());
			frequency_rootElement = parametrizedModel.createElement(null,
					FREQUENCY_ELEMENT, NetworkRelationshipTypes.FREQUENCY_ROOT);
		}
		clearElementMaps();
		...
		Integer hoppingType = (Integer) getSynonymValueWithAutoparse(
				HOPPING_TYPE, value);
		...
		if (bcchByte instanceof Integer) {
			...
			isBcch = bcchByte.toString().equalsIgnoreCase(
		...
		Integer arfcn = null;
		for (int i = 0; i < 63; i++) {
			arfcn = (Integer) getSynonymValueWithAutoparse(ARFCN + " "
					+ (i + 1), value);
			if (arfcn != null) {
				arfcnArray[i] = arfcn;
			}
	 ...
	 * @param arfcnArray2
	 */
	private void collectFrequencyElement(String hsn, Integer maio,
			Integer[] arfcnArray2) {
	...
	private void createOrUpdateTRX() throws AWEException {
		IDataElement findedSector = parametrizedModel
				.findElement(SECTOR_ELEMENT);
		if (findedSector == null) {
			LOGGER.warn("Cann't find sector on line " + lineCounter);
		}
		Iterable<IDataElement> sectorsTrx = parametrizedModel
		...
		if (findedTrx == null) {
			findedTrx = parametrizedModel.createElement(findedSector,
					TRX_ELEMENT);
		} else {
	...
	/**
	 * find or create frequecny element for required trx and update relationship
	 * between them;
	 * 
	 * @param findedTrx
	 * @throws AWEException
	 */
	private void createOrUpdateFrequencyElement(IDataElement findedTrx)
			throws AWEException {
		...
		if (frequencyElement != null && !frequencyElement.hasNext()) {
			parametrizedModel.createElement(findedTrx, FREQUENCY_ELEMENT,
					NetworkRelationshipTypes.ENTRY_PLAN);
		} else {
			parametrizedModel.completeProperties(frequencyElement.next(),
					FREQUENCY_ELEMENT, true);
		}
	...
	private void collectSectorElement(String sectorName) {
		...
		SECTOR_ELEMENT.put(NewAbstractService.TYPE,
	 ...
	 * @param arfcn2
	 */
	private void collectTRXElement(String subcell, Integer trxId, String band,
			String extended, boolean isBcch, Integer hoppingType) {
		...
		TRX_ELEMENT.put(NewAbstractService.TYPE,
