...
public class NewNetworkSaver extends
	...
	private void createMSC(List<String> row) {
		...
		if (fileSynonyms.get(DataLoadPreferenceManager.MSC) == null
				|| row.get(columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.MSC))) == null
				|| row.get(
						columnSynonyms.get(fileSynonyms
								.get(DataLoadPreferenceManager.MSC)))
						.equals("")) {
			createBSC(null, row);
			return;
		}
		...
		mscProperty.put(INeoConstants.PROPERTY_NAME_NAME, row
	...
	private void createBSC(IDataElement root, List<String> row) {
		...
		if (fileSynonyms.get(DataLoadPreferenceManager.BSC) == null
				|| row.get(columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.BSC))) == null
				|| row.get(
						columnSynonyms.get(fileSynonyms
								.get(DataLoadPreferenceManager.BSC)))
						.equals("")) {
			if (root == null) {
				createCity(null, row);
			} else {
		...
		bscProperty.put(INeoConstants.PROPERTY_NAME_NAME, row
				.get(columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.BSC))));
	...
	private void createCity(IDataElement root, List<String> row) {
		if (fileSynonyms.get(DataLoadPreferenceManager.CITY) == null
				|| row.get(columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.CITY))) == null
				|| StringUtils.isEmpty(row.get(columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.CITY).toString())))) {
			if (root == null) {
		...
		cityPropMap.put(INeoConstants.PROPERTY_NAME_NAME, row
				.get(columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.CITY))));
	...
	private void createSite(IDataElement root, List<String> row) {
		if (row.get(columnSynonyms.get(fileSynonyms
		...
		siteMap.put(INeoConstants.PROPERTY_LON_NAME, row.get(columnSynonyms
				.get(fileSynonyms.get(INeoConstants.PROPERTY_LON_NAME))));
		siteMap.put(INeoConstants.PROPERTY_LAT_NAME, row.get(columnSynonyms
				.get(fileSynonyms.get(INeoConstants.PROPERTY_LAT_NAME))));
		if (fileSynonyms.get(DataLoadPreferenceManager.SITE) == null
				|| row.get(
						columnSynonyms.get(fileSynonyms
								.get(DataLoadPreferenceManager.SITE))).equals(
						StringUtils.EMPTY)) {

			if (fileSynonyms.get(DataLoadPreferenceManager.SECTOR) != null
					&& !row.get(
							columnSynonyms.get(fileSynonyms
									.get(DataLoadPreferenceManager.SECTOR)))
							.equals(StringUtils.EMPTY)) {
				String siteName = row.get(columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.SECTOR)));
			...
			siteMap.put(INeoConstants.PROPERTY_NAME_NAME, row
					.get(columnSynonyms.get(fileSynonyms
							.get(DataLoadPreferenceManager.SITE))));
			row.set(columnSynonyms.get(fileSynonyms
					.get(DataLoadPreferenceManager.SITE)), null);
		}
		...
		for (String head : headers) {
			if (row.get(columnSynonyms.get(head)) != null
				...
				sectorMap.put(head.toLowerCase(),
						row.get(columnSynonyms.get(head)));
			}
		}

		String sectorName = row.get(columnSynonyms.get(fileSynonyms
				.get(DataLoadPreferenceManager.SECTOR))) != null ? row.get(
				columnSynonyms.get(fileSynonyms
						.get(DataLoadPreferenceManager.SECTOR))).toString()
				: "";
	...
	private void makeAppropriationWithSynonyms(List<String> keySet) {
		...
		for (String header : keySet) {
			for (String posibleHeader : preferenceManager
					.getNetworkPosibleValues().keySet()) {
				for (String mask : preferenceManager.getNetworkPosibleValues()
						.get(posibleHeader)) {
