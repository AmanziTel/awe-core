 ...
 */
public class NewNetworkSaver extends
		AbstractSaver<NetworkModel, CSVContainer, ConfigurationDataImpl> {
	...
	/**
	 * contains appropriation of header synonyms and name inDB</br> <b>key</b>-
	 * name in db , <b>value</b>-file header key
	 */
	...
	protected NewNetworkSaver(INetworkModel model,
	...
	/**
	 * find or create BSC node from row properties and pass the action down the
	 * chain, for creation BSC->CITY->SITE->SECTOR structure
	 */
	private void createMSC(List<String> row) throws AWEException {
		Map<String, Object> mscProperty = new HashMap<String, Object>();
		if (fileSynonyms.get(MSC) == null
				|| row.get(columnSynonyms.get(fileSynonyms.get(MSC))) == null
				|| row.get(columnSynonyms.get(fileSynonyms.get(MSC)))
						.equals("")) {
			createBSC(null, row);
			return;
		}

		mscProperty.put(INeoConstants.PROPERTY_TYPE_NAME,
				NetworkElementNodeType.MSC.getId());
		mscProperty.put(INeoConstants.PROPERTY_NAME_NAME,
				autoParse(row.get(columnSynonyms.get(fileSynonyms.get(MSC)))));
		...
		if (findedElement == null) {
			...
			addedDatasetSynonyms(model, NetworkElementNodeType.MSC,
	...
	/**
	 * find or create BSC node from row properties and pass the action down the
	 * chain, for creation CITY->SITE->SECTOR structure
	 * 
	 * @param row
	 * @throws AWEException
	 */
	private void createBSC(IDataElement root, List<String> row)
		...
		if (fileSynonyms.get(BSC) == null
				|| row.get(columnSynonyms.get(fileSynonyms.get(BSC))) == null
				|| row.get(columnSynonyms.get(fileSynonyms.get(BSC)))
						.equals("")) {
			if (root == null) {
				createCity(null, row);
			} else {
		...
		bscProperty.put(INeoConstants.PROPERTY_TYPE_NAME,
				NetworkElementNodeType.BSC.getId());
		bscProperty.put(INeoConstants.PROPERTY_NAME_NAME,
				autoParse(row.get(columnSynonyms.get(fileSynonyms.get(BSC)))));
		IDataElement findedElement;
		...
		if (findedElement == null) {
			if (root != null) {
				...
				findedElement = model.createElement(rootDataElement,
						bscProperty);
			}
			addedDatasetSynonyms(model, NetworkElementNodeType.BSC,
					INeoConstants.PROPERTY_NAME_NAME,
	 ...
	 * chain, for creation SITE->SECTOR nodes
	 * 
	 * @param row
	 * @throws AWEException
	 */
	private void createCity(IDataElement root, List<String> row)
			throws AWEException {
		if (fileSynonyms.get(CITY) == null
				|| row.get(columnSynonyms.get(fileSynonyms.get(CITY))) == null
				|| StringUtils.isEmpty(row.get(columnSynonyms.get(fileSynonyms
						.get(CITY).toString())))) {
			if (root == null) {
		...
		cityPropMap.put(INeoConstants.PROPERTY_TYPE_NAME,
				NetworkElementNodeType.CITY.getId());
		cityPropMap.put(INeoConstants.PROPERTY_NAME_NAME,
				autoParse(row.get(columnSynonyms.get(fileSynonyms.get(CITY)))));
		...
		if (findedElement == null) {
			...
			addedDatasetSynonyms(model, NetworkElementNodeType.CITY,
					INeoConstants.PROPERTY_NAME_NAME,
	...
	private void createSite(IDataElement root, List<String> row)
			throws AWEException {
		if ((fileSynonyms.get(INeoConstants.PROPERTY_LAT_NAME) == null || row
				.get(columnSynonyms.get(fileSynonyms
						.get(INeoConstants.PROPERTY_LAT_NAME))) == null)
				|| StringUtils.isEmpty(row.get(columnSynonyms.get(fileSynonyms
						.get(INeoConstants.PROPERTY_LAT_NAME).toString())))
				|| (fileSynonyms.get(INeoConstants.PROPERTY_LON_NAME) == null || row
						.get(columnSynonyms.get(fileSynonyms
								.get(INeoConstants.PROPERTY_LON_NAME))) == null)
				|| StringUtils.isEmpty(row.get(columnSynonyms.get(fileSynonyms
						.get(INeoConstants.PROPERTY_LON_NAME).toString())))) {
			LOGGER.info("Missing site name on line:" + lineCounter);
			return;
		}

		Map<String, Object> siteMap = new HashMap<String, Object>();
		siteMap.put(INeoConstants.PROPERTY_TYPE_NAME,
				NetworkElementNodeType.SITE.getId());
		siteMap.put(INeoConstants.PROPERTY_LON_NAME, autoParse(row
				.get(columnSynonyms.get(fileSynonyms
						.get(INeoConstants.PROPERTY_LON_NAME)))));
		siteMap.put(INeoConstants.PROPERTY_LAT_NAME, autoParse(row
				.get(columnSynonyms.get(fileSynonyms
						.get(INeoConstants.PROPERTY_LAT_NAME)))));
		String siteName;
		if (fileSynonyms.get(SITE) == null
				|| row.get(columnSynonyms.get(fileSynonyms.get(SITE))).equals(
						StringUtils.EMPTY)) {
			if (fileSynonyms.get(SECTOR) != null
					&& !row.get(columnSynonyms.get(fileSynonyms.get(SECTOR)))
							.equals(StringUtils.EMPTY)) {
				siteName = row
						.get(columnSynonyms.get(fileSynonyms.get(SECTOR)));
				siteMap.put(INeoConstants.PROPERTY_NAME_NAME,
						autoParse(siteName.substring(0, siteName.length() - 1)));
			} else {
			...
			siteMap.put(INeoConstants.PROPERTY_NAME_NAME, autoParse(row
					.get(columnSynonyms.get(fileSynonyms.get(SITE)))));
		...
		if (findedElement == null) {
			...
			addedDatasetSynonyms(
					...
					INeoConstants.PROPERTY_NAME_NAME,
					columnSynonyms.get(fileSynonyms.get(SITE)) == null ? SITE
							: headers.get(columnSynonyms.get(fileSynonyms
									.get(SITE))));
			addedDatasetSynonyms(model, NetworkElementNodeType.SITE,
					INeoConstants.PROPERTY_LON_NAME, headers.get(columnSynonyms
							.get(fileSynonyms
									.get(INeoConstants.PROPERTY_LON_NAME))));
			addedDatasetSynonyms(model, NetworkElementNodeType.SITE,
					INeoConstants.PROPERTY_LAT_NAME, headers.get(columnSynonyms
							.get(fileSynonyms
									.get(INeoConstants.PROPERTY_LAT_NAME))));
		}
		row.set(columnSynonyms.get(fileSynonyms
				.get(INeoConstants.PROPERTY_LON_NAME)), null);
		row.set(columnSynonyms.get(fileSynonyms
				.get(INeoConstants.PROPERTY_LAT_NAME)), null);
	...
	private void createSector(IDataElement root, List<String> row)
		...
		for (String head : headers) {
			if (row.get(columnSynonyms.get(head)) != null
					&& !StringUtils.isEmpty(row.get(columnSynonyms.get(head)))
					&& head != fileSynonyms.get(SECTOR)) {
				sectorMap.put(head.toLowerCase(),
						autoParse(row.get(columnSynonyms.get(head))));
				if (fileSynonyms.containsValue(head)) {
					for (String key : fileSynonyms.keySet()) {
						if (head.equals(fileSynonyms.get(key))) {
							addedDatasetSynonyms(model,
									NetworkElementNodeType.SECTOR, key, head);
						}
					}
				}
			}
		...
		String sectorName = row
				.get(columnSynonyms.get(fileSynonyms.get(SECTOR))) != null ? row
				.get(columnSynonyms.get(fileSynonyms.get(SECTOR))).toString()
				: "";
		String ci = sectorMap.containsKey("ci") ? sectorMap.get("ci")
				.toString() : "";
		String lac = sectorMap.containsKey("lac") ? sectorMap.get("lac")
				.toString() : "";
		if ((ci == null || StringUtils.isEmpty(ci))
				&& (lac == null || StringUtils.isEmpty(lac))
		...
		sectorMap.put(INeoConstants.PROPERTY_TYPE_NAME,
		...
		if (findedElement == null) {
			...
			addedDatasetSynonyms(model, NetworkElementNodeType.SECTOR,
					INeoConstants.PROPERTY_NAME_NAME,
					headers.get(getHeaderId(fileSynonyms.get(SECTOR))));
		} else {
			LOGGER.info("sector " + sectorMap.get(CI_LAC.toLowerCase())
					+ " is already exist; line: " + lineCounter);
		}
	}

	@Override
	public void init(ConfigurationDataImpl configuration,
			CSVContainer dataElement) {
		...
		commitTx();
		try {
			rootElement.put(INeoConstants.PROPERTY_NAME_NAME, configuration
					.getDatasetNames().get(CONFIG_VALUE_NETWORK));
			model = getActiveProject().getNetwork(
					configuration.getDatasetNames().get(CONFIG_VALUE_NETWORK));
			rootDataElement = new DataElement(model.getRootNode());
			modelMap.put(
					configuration.getDatasetNames().get(CONFIG_VALUE_NETWORK),
					model);
