...
public class Nemo1xSaver extends Nemo2xSaver {
	...
	protected Nemo1xSaver(IDriveModel model, ConfigurationDataImpl config,
	...
	protected void commonLinePreparationActions(CSVContainer dataElement)
		...
		if (workDate == null) {
			...
			Date date;
			try {
				date = EVENT_DATE_FORMAT.parse(dataElement.getFirstLine()
						.split(NEMO_FIRST_LINE_SPLIT)[2]);
			} catch (Exception e) {
	...
	protected void saveLine(List<String> value) throws AWEException {
		...
		Object longitude = autoParse(IDriveModel.LONGITUDE,
				value.get(LONGITUDE_INDEX));
		Object latitude = autoParse(IDriveModel.LATITUDE,
				value.get(LATITUDE_INDEX));
		...
		Map<String, Object> parsedParameters = analyseKnownParameters(value,
				event, contextId, parameters);
		if (parsedParameters == null) {
			return;
		}
		if (isCorrect(latitude) && (Double) latitude != INCORRECT_LAT_LON_VALUE
				&& isCorrect(longitude)
