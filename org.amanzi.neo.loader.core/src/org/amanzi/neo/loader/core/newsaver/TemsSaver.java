...
public class TemsSaver extends AbstractDriveSaver {
	...
	protected TemsSaver(IDriveModel model, IDriveModel virtualModel,
	...
	protected void saveLine(List<String> value) throws AWEException {
		...
		collectMElement(time, latitude, longitude, message_type, timestamp,
				value, ms, event);

		removeEmpty(params);
		collectRemainProperties(params, value);
		IDataElement createdMeasurment = addMeasurement(parametrizedModel,
				params);
	 ...
	 * @param event
	 */
	private void collectMElement(Object time, Double latitude,
			Double longitude, String message_type, Long timestamp,
		...
		params.put(
				NewNetworkService.CELL_INDEX,
				getSynonymValueWithAutoparse(NewNetworkService.CELL_INDEX,
						value));
	...
	private void createVirtualModelElement(List<String> value, String ms,
			String time, String event, Long timestamp) throws AWEException {
		...
		try {
			// TODO: LN: what is '+1'
			channel = (Integer) getSynonymValueWithAutoparse(
					ALL_PILOT_SET_CHANNEL + INDEX_FIRST_PILOT_ELEMNT, value);
			pn_code = (Integer) getSynonymValueWithAutoparse(ALL_PILOT_SET_PN
					+ INDEX_FIRST_PILOT_ELEMNT, value);
			ec_io = (Integer) getSynonymValueWithAutoparse(ALL_PILOT_SET_EC_IO
					+ INDEX_FIRST_PILOT_ELEMNT, value);
			measurement_count = (Integer) getSynonymValueWithAutoparse(
					ALL_PILOT_SET_COUNT, value);
		} catch (Exception e) {
			LOGGER.error("Failed to parse a field on line " + lineCounter
					+ ": " + e.getMessage());
			return;
		}

		if (measurement_count > MAXIMUM_MEASURMENT_COUNT) {
			LOGGER.error("Measurement count " + measurement_count + " > "
					+ MAXIMUM_MEASURMENT_COUNT);
		...
		if (measurement_count > 0
			...
			for (int i = 1; i <= measurement_count; i++) {
				...
				try {
					ec_io = (Integer) getSynonymValuewithAutoparse(
							ALL_PILOT_SET_EC_IO + i, value);
					channel = (Integer) getSynonymValuewithAutoparse(
							ALL_PILOT_SET_CHANNEL + i, value);
					pn_code = (Integer) getSynonymValuewithAutoparse(
							ALL_PILOT_SET_PN + i, value);
					String chan_code = StringUtils.EMPTY + channel + "\t"
							+ pn_code;
					...
					tempSignal.getChanarray()[0] += Math.pow(10.0,
							((ec_io) / 10.0));
					tempSignal.getChanarray()[1] += 1;
				} catch (Exception e) {
					LOGGER.error("Error parsing column " + i
							+ " for EC/IO, Channel or PN: " + e.getMessage(), e);
				}
			}
		}
		if (!signals.isEmpty()) {
			...
			for (Signal signal : signals) {
				sorted_signals.put(
						signal.getChanarray()[1] / signal.getChanarray()[0],
						signal);
			}
			for (Map.Entry<Float, Signal> entry : sorted_signals.entrySet()) {
				...
				IDataElement virtualMeasurment = addMeasurement(virtualModel,
						params);
	 ...
	 * @param timestamp
	 */
	private void collectMsElement(String[] cc, float dbm, double mw,
			Long timestamp) {
		...
		params.put(NewAbstractService.NAME,
				autoParse(NewAbstractService.NAME, cc[1]));
		params.put(DBM, dbm);
		params.put(MW, mw);
		params.put(TIMESTAMP, timestamp);
	}

	private IDataElement addMeasurement(IDriveModel model,
			Map<String, Object> properties) throws AWEException {
