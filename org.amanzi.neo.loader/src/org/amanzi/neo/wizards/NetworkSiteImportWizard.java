    ...
    public boolean performFinish() {
        ...
        Job job = new Job("Load Network Site'" + (new File(mainPage.getFileName())).getName() + "'") {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                NetworkSiteLoader networkLoader;
                try {

                    networkLoader = new NetworkSiteLoader(mainPage.getNetworkNode(), mainPage.getFileName(), display);
                    networkLoader.run(monitor);
                    networkLoader.printStats(false);
                } catch (IOException e) {
                    NeoCorePlugin.error("Error loading Network site file", e);
