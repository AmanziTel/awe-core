...
public class RubyEditor extends RubyAbstractEditor implements IRubyReconcilingListener
{
	...
	/**
	 * Mutex for the reconciler. See https://bugs.eclipse.org/bugs/show_bug.cgi?id=63898 for a description of the
	 * problem.
	 * <p>
	...
	protected void createActions()
	{
		...
		fActionGroups = new CompositeActionGroup(new ActionGroup[] { oeg = new OpenEditorActionGroup(this),
				ovg = new OpenViewActionGroup(this), rsg = new RubySearchActionGroup(this) });
		...
		ResourceAction resAction = new TextOperationAction(RubyEditorMessages.getBundleForConstructedKeys(),
				"ShowRDoc.", this, ISourceViewer.INFORMATION, true); //$NON-NLS-1$
		resAction = new InformationDispatchAction(RubyEditorMessages.getBundleForConstructedKeys(),
				"ShowRDoc.", (TextOperationAction) resAction); //$NON-NLS-1$
		...
		action = new ToggleCommentAction(RubyPlugin.getDefault().getPluginProperties(), "ToggleCommentAction.", this); //$NON-NLS-1$
		...
		action = new RubySelectMarkerRulerAction2(RubyEditorMessages.getBundleForConstructedKeys(),
		...
		action = new TextOperationAction(RubyEditorMessages.getBundleForConstructedKeys(),
				"ShowOutline.", this, RubySourceViewer.SHOW_OUTLINE, true); //$NON-NLS-1$
		...
		action = new TextOperationAction(RubyEditorMessages.getBundleForConstructedKeys(),
				"OpenStructure.", this, RubySourceViewer.OPEN_STRUCTURE, true); //$NON-NLS-1$
		...
		action = new TextOperationAction(RubyEditorMessages.getBundleForConstructedKeys(),
				"OpenHierarchy.", this, RubySourceViewer.SHOW_HIERARCHY, true); //$NON-NLS-1$
	...
	/*
	 * @see
	 * org.eclipse.ui.texteditor.AbstractTextEditor#rulerContextMenuAboutToShow(org.eclipse.jface.action.IMenuManager)
	 */
	 ...
	 * CHANGED TO WORK WITH 3.2 (Non-breaking in 3.1) Method couldn't be restricted to private, changed to protected
	 * 
	 * @param annotation
	 *            the annotation
	...
	/**
	 * Jumps to the next enabled annotation according to the given direction. An annotation type is enabled if it is
	 * configured to be in the Next/Previous tool bar drop down menu and if it is checked. CHANGED TO WORK WITH 3.2
	 * (Non-breaking in 3.1) Annotation type must be returned
	 * 
	 * @param forward
			...
			for (int j = 0; j < elements.length; j++)
			{
				...
				if (selectionEnabler.isEnabledForSelection(this.getSelectionProvider().getSelection()))
				{
					try
					{
						...
						{
							String message = "The editorPopupExtender" + element.getName() + " is of type "
									+ menuExtender.getClass().getName() + " , but should be of type ActionGroup";
							RubyPlugin.log(IStatus.ERROR, message, null);
							continue;
						}
						ActionGroup menuExtenderActionGroup = (ActionGroup) menuExtender;
						menuExtenderActionGroup
								.setContext(new ActionContext(this.getSelectionProvider().getSelection()));
	...
	protected ISourceViewer createRubySourceViewer(Composite parent, IVerticalRuler verticalRuler,
			IOverviewRuler overviewRuler, boolean isOverviewRulerVisible, int styles, IPreferenceStore store)
	{
		ISourceViewer viewer = new AdaptedSourceViewer(parent, verticalRuler, overviewRuler, isOverviewRulerVisible,
				styles, store);
		...
		/*
		 * This is a performance optimization to reduce the computation of the text presentation triggered by {@link
		 * #setVisibleDocument(IDocument)}
		 */
		// if (rubySourceViewer != null && isFoldingEnabled() && (store == null ||
	...
	/**
	 * Returns the mutex for the reconciler. See https://bugs.eclipse.org/bugs/show_bug.cgi?id=63898 for a description
	 * of the problem.
	 * <p>
	 * TODO remove once the underlying problem is solved.
	 * </p>
	...
	private class ExitPolicy implements IExitPolicy
	{
		...
		/*
		 * @see
		 * org.eclipse.jdt.internal.ui.text.link.LinkedPositionUI.ExitPolicy#doExit(org.eclipse.jdt.internal.ui.text
		 * .link.LinkedPositionManager, org.eclipse.swt.events.VerifyEvent, int, int)
		 */
		...
		public void verifyKey(VerifyEvent event)
		{
			...
			try
			{
				...
				switch (event.character)
				{
					...
					case '{':
						if (!fCloseBraces || nextToken == Symbols.TokenLBRACE || nextToken == Symbols.TokenIDENT
								|| next != null && next.length() > 1)
							return;
						break;
				...
				ITypedRegion partition = TextUtilities.getPartition(document, IRubyPartitions.RUBY_PARTITIONING,
						offset - 1, true);
				if (event.character == '{' && previous != null && previous.endsWith("#") && partition != null && partition.getType() != null)
				{
					if (!IDocument.DEFAULT_CONTENT_TYPE.equals(partition.getType())
							&& !IRubyPartitions.RUBY_STRING.equals(partition.getType()))
						return;
				}
				else
				{
	...
	/**
	 * Returns the most narrow element including the given offset. If <code>reconcile</code> is <code>true</code> the
	 * editor's input element is reconciled in advance. If it is <code>false</code> this method only returns a result if
	 * the editor's input element does not need to be reconciled.
	 * 
	 * @param offset
	...
	/**
	 * Runner that will toggle folding either instantly (if the editor is visible) or the next time it becomes visible.
	 * If a runner is started when there is already one registered, the registered one is canceled as toggling folding
	 * twice is a no-op.
	 * <p>
	 * The access to the fFoldingRunner field is not thread-safe, it is assumed that <code>runWhenNextVisible</code> is
	...
	/*
	 * @see org.rubypeople.rdt.internal.ui.text.ruby.IRubyReconcilingListener#reconciled(RootNode, boolean,
	 * IProgressMonitor)
	 * @since 1.0
	 */
	...
	class InformationDispatchAction extends TextEditorAction
	{
		...
		private boolean makeAnnotationHoverFocusable(ISourceViewer sourceViewer, IAnnotationHover annotationHover)
		{
			...
			try
			{
				...
				String contentType = TextUtilities.getContentType(document, IRubyPartitions.RUBY_PARTITIONING, offset,
						true);

				IInformationControlCreator controlCreator = null;

				/*
				 * XXX: This is a hack to avoid API changes at the end of 3.2, and should be fixed for 3.3, see:
			...
			IInformationProviderExtension2
	{
		...
		/*
		 * @seeorg.eclipse.jface.text.information.IInformationProviderExtension#getInformation2(org.eclipse.jface.text.
