...
public class RubyPartitionScanner implements IPartitionTokenScanner
{
	...
	public static final String[] LEGAL_CONTENT_TYPES = { RUBY_DEFAULT, RUBY_MULTI_LINE_COMMENT,
			RUBY_SINGLE_LINE_COMMENT, RUBY_REGULAR_EXPRESSION, RUBY_STRING, RUBY_COMMAND };
	...
	public void setPartialRange(IDocument document, int offset, int length, String contentType, int partitionOffset)
	{
		...
		try
		{
			...
			if (fContents.startsWith("}") && offset > 1 && fContents.length() >= 2)
			{
	...
	private void setLexerPastDynamicSectionOfString() throws IOException
	{
		...
		{
			fakeContents.append(new String(fContents.substring((fOffset - origOffset)))); // BLAH removed + 1 from end
																							// here
		}
	...
	private IToken getToken(int i)
	{
		...
		switch (i)
		{
			...
			case Tokens.tSTRING_BEG:
				...
				else if (fOpeningString.startsWith("<<"))
				{ // here-doc
					fOpeningString += "\n";
				}
				fContentType = RUBY_STRING;
			...
			case Tokens.tREGEXP_END:
				...
				return new Token(RUBY_REGULAR_EXPRESSION);
			case Tokens.tSYMBEG:
				// TODO Check for '"' after it
				int nextCharOffset = (fOffset +2);
				...
				{
					fOpeningString = "\"";
					push(new QueuedToken(new Token(RUBY_STRING), nextCharOffset, 1));
					fContentType = RUBY_STRING;
				}
				return new Token(RUBY_DEFAULT);
			default:
	...
	private boolean isCommentMultiLine(CommentNode comment)
	{
		...
		if (src != null && src.startsWith(BEGIN))
			return true;
		return false;
	}
