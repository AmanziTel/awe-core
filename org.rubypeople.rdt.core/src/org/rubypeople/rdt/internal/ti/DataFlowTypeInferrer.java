...
public class DataFlowTypeInferrer implements ITypeInferrer
{
	...
	public List<ITypeGuess> infer(String source, int offset)
	{
		...
		guesses = combineSameGuesses(guesses);
		return guesses;
	}

	private List<ITypeGuess> combineSameGuesses(List<ITypeGuess> guesses)
	{
		// TODO If we have two String guesses at 50%, add them back together as one String guess at 100%!
		Map<String, Integer> combined = new HashMap<String, Integer>();
		for (ITypeGuess typeGuess : guesses)
		{
			Integer percent = combined.get(typeGuess.getType());
			if (percent == null)
			{
		...
		List<ITypeGuess> combinedGuesses = new ArrayList<ITypeGuess>(combined.size());
		for (String type : combined.keySet())
		{
			combinedGuesses.add(new BasicTypeGuess(type, combined.get(type)));
		}
		return combinedGuesses;
	}

	/**
	 * Redistribute the confidence percentages. I.e. if guesses contains three guesses, each at 100%, they will now each
	 * be 33%.
	 * 
	 * @param guesses
	 *            Guesses to redistribute
	...
	{
		return (node instanceof Colon2Node) || (node instanceof ConstNode)
				|| (null != LiteralNodeTypeNames.get(node.getClass().getSimpleName()));
	}
	...
	{
		return MethodInvocationLocator.Instance().findMethodInvocations(rootNode, typeName, methodName,
				new DataFlowTypeInferrer());
	}
	...
	private List<Node> findRetvalExprs(Node methodNode)
	{
		...
		sysout("Found " + retvalExprs.size() + " + retval exprs in method "
				+ helper.getMethodDefinitionNodeName(methodNode));
	...
	{
		Node enclosingScopeNode = ClosestSpanningNodeLocator.Instance().findClosestSpanner(rootNode,
				node.getPosition().getStartOffset(), new INodeAcceptor()
				{
	...
	{
		Node enclosingTypeNode = ClosestSpanningNodeLocator.Instance().findClosestSpanner(rootNode,
				node.getPosition().getStartOffset(), new INodeAcceptor()
				{
					public boolean doesAccept(Node node)
					{
	...
	private List<ITypeGuess> getLocalVarReferenceNodeTypes(LocalVarNode node)
	{
		...
		List<Node> localAssignsIntoNode = ScopedNodeLocator.Instance().findNodesInScope(enclosingScopeNode,
				new INodeAcceptor()
				{
	...
	private List<ITypeGuess> getClassVarReferenceNodeTypes(ClassVarNode node)
	{
		...
		List<Node> classAsgnNodes = ScopedNodeLocator.Instance().findNodesInScope(enclosingTypeNode,
				new INodeAcceptor()
				{
					public boolean doesAccept(Node acceptNode)
					{
		...
		// TODO: class Klass;@@x=5;@@x=6;@@x;end # @@x=5 is parsed as a ClassDeclNode, and so is @@x=6. Do
		// ClassAsgnNodes ever pop up???
	...
	private List<ITypeGuess> getCallNodeTypes(Node node)
	{
		...
		// heuristic for common method which should return known types, i.e to_s, to_i
